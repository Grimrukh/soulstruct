...
class EVSInstructionCompiler(_BaseCompiler):
    ...
    def RunCommonEvent(self, event_id, slot=0, args=(0,), arg_types="", event_layers=None):
        ...
        instruction is used to initialize LOCAL map events as 'common' events. I'm not sure if this has any purpose, or
        if it's just an artifact of FromSoft copy-pasting between EMEVD files and forgetting to change the Run call.
        ...
        WARNING: Float arguments are difficult to detect without the signature of the called event -- for example, if
        you are calling a `common_func` event without the EVS module to inspect. This is fine if you NEVER have that
        module available, as the unsigned integer version of the float will be repacked to the same bytes, but if you
        add the `common_func` EMEVD *afterward* and give the float argument an unsigned integer, you will instead end up
        calling it with a (probably high magnitude) float!
        """
        if not arg_types:
        ...
        if len(args) != len(arg_types):
            raise ValueError(
                f"Number of event arguments ({len(args)}) does not match length "
                f"of argument type string ({len(arg_types)}) in `RunCommonEvent`:\n"
                f"   Args: {args}\n"
    ...
    def EnableAssetActivation(self, obj: AssetTyping, obj_act_id: int, relative_index=None):
        """
    ...
    def IfPlayerHasTalisman(self, condition: int, rune: AccessoryTyping, including_storage: bool = False):
        return self.IfPlayerItemState(condition, True, rune, ItemType.Talisman, including_storage)
    ...
    def IfPlayerDoesNotHaveTalisman(self, condition: int, ring: AccessoryTyping, including_storage: bool = False):
        return self.IfPlayerItemState(condition, False, ring, ItemType.Talisman, including_storage)
