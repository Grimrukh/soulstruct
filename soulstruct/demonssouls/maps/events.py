...
class EventHeaderStruct(MSBHeaderStruct):
    ...
    _pad1: bytes = field(**BinaryPad(8))  # extra 4 bytes compared to DS1
...
class EventDataStruct(MSBBinaryStruct):
    ...
    entity_id: int
    _pad1: bytes = field(**BinaryPad(4))  # always zero, unlike `unknowns` bytes in DS1
...
class SpawnerEventDataStruct(MSBBinaryStruct):
    ...
    _pad1: bytes = field(**BinaryPad(0x1F))
    ...
    _pad2: bytes = field(**BinaryPad(0x40))
