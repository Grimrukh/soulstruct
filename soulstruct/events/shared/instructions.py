...
def SetTeamType(character: CharacterTyping, new_team: Union[TeamType, int]):
...
def SetNPCPartHealth(character: CharacterTyping, npc_part_id: int, desired_hp: int, overwrite_max: bool):
...
def PostDestruction(obj: ObjectTyping, slot: int):
...
def SetObjectInvulnerability(obj: ObjectTyping, state: bool):
    """ 1 = invulnerable. """
...
def PlaySoundEffect(
    anchor_entity: CoordEntityTyping, sound_type: Union[None, SoundType, int] = None, sound_id: Union[Sound, int] = -1
):
...
def PlayCutscene(
    ...
    move_to_map: Optional[MapTyping] = None,
    move_to_region: Optional[RegionTyping] = None,
    rotation: int = 0,
...
def PlayCutsceneAndMovePlayer(
    cutscene_id: int, playback_method: CutsceneType, region: RegionTyping, game_map: MapTyping
):
...
def PlayCutsceneAndMoveSpecificPlayer(
    cutscene_id: int, playback_method: CutsceneType, region: RegionTyping, game_map: MapTyping, player_id: int
):
...
def WarpToMap(game_map: MapTyping, destination_player_id=-1):
...
def TerminateIfMapPresenceState(event_end_type: EventEndType, state: bool, game_map: MapTyping):
...
def TerminateIfFlagRangeState(
    event_end_type: EventEndType, state: RangeState, flag_type: FlagType, flag_range: FlagRangeTyping
):
...
def IfPlayerItemState(
    condition: int, state: bool, item: ItemTyping, item_type: Optional[ItemType] = None, including_box: bool = True
):
...
def IfPlayerHasItem(condition: int, item: ItemTyping, item_type: Optional[ItemType] = None, including_box: bool = True):
...
def IfPlayerDoesNotHaveItem(
    condition: int, item: ItemTyping, item_type: Optional[ItemType] = None, including_box: bool = True
):
...
def IfItemDropped(condition: int, item: ItemTyping, item_type: Union[ItemType, int, None] = None):
...
def TerminateIfObjectDestructionState(event_end_type: EventEndType, obj: ObjectTyping, state: bool):
...
def IfAttacked(condition: int, attacked_entity: CharacterTyping, attacking_character: CharacterTyping):
