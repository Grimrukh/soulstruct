...
def Event11600001():
    ...
    PlayCutscene(
        160010, skippable=True, fade_out=False, player_id=PLAYER, move_to_region=1602120, move_to_map=NEW_LONDO_RUINS
    )
    SkipLines(4)
    SkipLinesIfClient(2)
    PlayCutscene(
        160010, skippable=False, fade_out=False, player_id=PLAYER, move_to_region=1602120, move_to_map=NEW_LONDO_RUINS
    )
    SkipLines(1)
...
def Event11605380():
    ...
    IfActionButton(
        1, prompt_text=10010403, anchor_entity=1602898, anchor_type=CoordEntityType.Region, boss_version=True
    )
    IfConditionTrue(0, input_condition=1)
    SkipLinesIfClient(1)
    NotifyBossBattleStart()
...
def Event11605381():
    ...
    IfCharacterType(1, PLAYER, CharacterType.WhitePhantom)
    IfActionButton(
        1, prompt_text=10010403, anchor_entity=1602898, anchor_type=CoordEntityType.Region, trigger_attribute=255
    )
    IfConditionTrue(0, input_condition=1)
    RotateToFaceEntity(PLAYER, 1602897)
    ForceAnimation(PLAYER, 7410)
