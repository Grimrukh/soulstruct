...
class BaseESD:
    ...
    class State:
        ...
        def unpack(cls, esd_type, esd_buffer, state_machine_offset, count):
            ...
            for d in struct_dicts:
                conditions = cls.Condition.unpack(
                    esd_type, esd_buffer, d["condition_pointers_offset"], count=d["condition_pointers_count"],
                )

                enter_commands = cls.Command.unpack(
                    esd_type, esd_buffer, d["enter_commands_offset"], count=d["enter_commands_count"],
                )

                exit_commands = cls.Command.unpack(
                    esd_type, esd_buffer, d["exit_commands_offset"], count=d["exit_commands_count"],
                )
                ...
                state_dict[d["index"]] = cls(
                    esd_type, d["index"], conditions, enter_commands, exit_commands, ongoing_commands
                )
        ...
        def html_title_bar(self):
            return (
                f'<br><div style="font-size:35px;font-weight:bold;margin-top:10px"><a name="ezstate_{self.index}">'
        ...
        def unpack(cls, esd_type, esd_buffer, condition_pointers_offset, count=1):
            ...
            for p in pointers:
                ...
                pass_commands = cls.Command.unpack(
                    esd_type, esd_buffer, d["pass_commands_offset"], count=d["pass_commands_count"],
                )

                subconditions = cls.unpack(
                    esd_type, esd_buffer, d["subcondition_pointers_offset"], count=d["subcondition_pointers_count"],
                )
        ...
        def to_html(self):
            ...
            if self.next_state_index != -1:
                string += state_fmt.format(
                    20 * (3 + self.__indent),
                    '---> <a href="#ezstate_{index}">State {index}' "</a>".format(index=self.next_state_index),
                )
            if self.pass_commands:
    ...
    class Command:
        ...
        ARG_STRUCT = BinaryStruct(("arg_ezl_offset", "i"), ("arg_ezl_size", "i"),)
        ...
        if isinstance(esd_source, bytes):
            ...
            if esd_source.is_dir():
                ...
                if esd_type not in {"chr", "talk"}:
                    raise ValueError(
                        "esd_type must be 'chr' or 'talk'. This cannot be safely auto-detected from"
                ...
                try:
                    ...
                    raise IOError(
                        f"Could not interpret file '{esd_source}' as binary ESD data. "
...
class _ESDPacker:
    ...
    def __update_offsets(self):
        ...
        self.offsets["condition_pointers"] = (
            self.offsets["command_args"] + len(self.command_args) * self.esd.Command.ARG_STRUCT.size
        )
        self.offsets["ezl"] = (
            self.offsets["condition_pointers"] + len(self.condition_pointers) * self.esd.Condition.POINTER_STRUCT.size
        )
    ...
    if long:
        ...
        class ESD(BaseESD):
            ...
            class Condition(BaseESD.Condition):
                ...
                POINTER_STRUCT = BinaryStruct(("condition_offset", "i"),)
                STATE_ID_FORMAT = "i"
