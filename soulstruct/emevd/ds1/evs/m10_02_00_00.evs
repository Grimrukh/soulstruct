...
def Preconstructor():
    ...
    RunEvent(11020899, slot=0, args=(50004000, 50004069))
    EnableFlag(11020120)
    EnableFlag(61020120)
...
def Event11020020():
    ...
    IfDialogPromptActivated(0, prompt_text=10010506, anchor_entity=1022120, anchor_type=MapEntityType.Region, facing_angle=0.0, max_distance=0.0, human_or_hollow_only=True)
    SetStandbyAnimationSettings(PLAYER, standby_animation=7816)
    ForceAnimation(PLAYER, 7815, wait_for_completion=True)
    EnableFlag(11025060)
    WaitFrames(3)
    IfDialogPromptActivated(1, prompt_text=10010507, anchor_entity=1022120, anchor_type=MapEntityType.Region, facing_angle=0.0, max_distance=0.0, human_or_hollow_only=True)
...
def Event11020021():
    ...
    Wait(20.0)
    PlayCutscene(100230, skippable=True, fade_out=False, player_id=PLAYER, move_to_region=1812110, move_to_map=UNDEAD_ASYLUM)
...
def Event11020106():
    ...
    IfDialogPromptActivated(1, prompt_text=10010182, anchor_entity=1021960, anchor_type=MapEntityType.Object, facing_angle=180.0, max_distance=3.4000000953674316, human_or_hollow_only=True)
    IfConditionTrue(0, input_condition=1)
    DisplayDialog(10010184, anchor_entity=1021960, display_distance=3.4000000953674316, button_type=ButtonType.Yes_or_No, number_buttons=NumberButtons.NoButton)
    Restart()
...
def Event11020351():
    ...
    End()
    PlayCutscene(180060, skippable=True, fade_out=False, player_id=PLAYER, move_to_region=1802110, move_to_map=KILN_OF_THE_FIRST_FLAME)
    WaitFrames(1)
    Restart()
...
def Event11020504(ARG_0_3: int, ARG_4_7: int):
    ...
    Move(ARG_0_3, destination=1022700, destination_type=MapEntityType.Region, model_point=-1, short_move=True)
...
def Event11026200():
    ...
    PlayCutscene(100240, skippable=True, fade_out=False, player_id=PLAYER, move_to_region=1802110, move_to_map=KILN_OF_THE_FIRST_FLAME)
...
def Event11026210():
    ...
    DisableFlag(1650)
    IfInsideMap(1, game_map=FIRELINK_SHRINE)
    IfEntityBeyondDistance(1, 6330, 10000, radius=30.0)
    IfFlagOff(1, 830)
    IfConditionTrue(0, input_condition=1)
    IfFlagOn(-1, 1643)
    IfFlagOn(-1, 1644)
    ...
    IfAttacked(0, 6330, attacking_character=10000)
