...
from soulstruct.emevd.ds3.enums import *
...
def IfCharacterRegionState(condition, entity: AnimatedInt, region: RegionInt, state: bool, min_target_count: int = 1):
    """ New argument with unknown purpose. Always 1 in vanilla files. Probably for debug. """
    instruction_info = (3, 2)
    return numeric_instruction(instruction_info, condition, state, entity, region, min_target_count)

def IfCharacterInsideRegion(condition: int, entity: AnimatedInt, region: RegionInt, min_target_count: int = 1):
    return IfCharacterRegionState(condition, entity, region, True, min_target_count)

def IfCharacterOutsideRegion(condition: int, entity: AnimatedInt, region: RegionInt, min_target_count: int = 1):
    return IfCharacterRegionState(condition, entity, region, False, min_target_count)

def IfPlayerInsideRegion(condition: int, region: RegionInt, min_target_count: int = 1):
    return IfCharacterRegionState(condition, PLAYER, region, True, min_target_count)

def IfPlayerOutsideRegion(condition: int, region: RegionInt, min_target_count: int = 1):
    return IfCharacterRegionState(condition, PLAYER, region, False, min_target_count)
...
def IfEntityDistanceState(condition: int, entity: MapEntityInt, other_entity: MapEntityInt, radius: float,
    ...
    """ Same new argument as region test, with unknown purpose, and again always 1 in EMEVD files. """
...
def IfPlayerWithinDistance(condition: int, other_entity: MapEntityInt, radius: float, min_target_count: int = 1):
    return IfEntityDistanceState(condition, PLAYER, other_entity, radius, True, min_target_count)

def IfPlayerBeyondDistance(condition: int, other_entity: MapEntityInt, radius: float, min_target_count: int = 1):
...
def IfMapCeremonyState(condition: int, state: bool, game_map: GameMap, ceremony_id: int):
...
def IfMapInCeremony(condition: int, game_map: GameMap, ceremony_id: int):
    return IfMapCeremonyState(condition, True, game_map, ceremony_id)

def IfMapNotInCeremony(condition: int, game_map: GameMap, ceremony_id: int):
...
# To make things event weirder, the 'target_count' usually seems to be a float.

def IfCharacterDeathState(condition: int, character: CharacterInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (4, 0)
    return numeric_instruction(instruction_info, condition, character, state, target_comparison_type, target_count)

def IfCharacterDead(condition: int, character: CharacterInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterDeathState(condition, character, True, target_comparison_type, target_count)

def IfCharacterAlive(condition: int, character: CharacterInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterDeathState(condition, character, False, target_comparison_type, target_count)


def IfHealthComparison(condition: int, character: CharacterInt, comparison_type: ComparisonType, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (4, 2)
    return numeric_instruction(instruction_info, condition, character, comparison_type, value, target_comparison_type, target_count)

def IfHealthEqual(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthComparison(condition, character, ComparisonType.Equal, value, target_comparison_type, target_count)

def IfHealthNotEqual(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthComparison(condition, character, ComparisonType.NotEqual, value, target_comparison_type, target_count)

def IfHealthGreaterThan(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthComparison(condition, character, ComparisonType.GreaterThan, value, target_comparison_type, target_count)

def IfHealthLessThan(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthComparison(condition, character, ComparisonType.LessThan, value, target_comparison_type, target_count)

def IfHealthGreaterThanOrEqual(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthComparison(condition, character, ComparisonType.GreaterThanOrEqual, value, target_comparison_type, target_count)

def IfHealthLessThanOrEqual(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthComparison(condition, character, ComparisonType.LessThanOrEqual, value, target_comparison_type, target_count)


def IfCharacterType(condition: int, character: CharacterInt, character_type: CharacterType, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (4, 3)
    return numeric_instruction(instruction_info, condition, character, character_type, target_comparison_type, target_count)

def IfCharacterHollow(condition: int, character: CharacterInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterType(condition, character, CharacterType.Hollow, target_comparison_type, target_count)

def IfCharacterHuman(condition: int, character: CharacterInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterType(condition, character, CharacterType.Human, target_comparison_type, target_count)


def IfCharacterSpecialEffectState(condition: int, character: CharacterInt, special_effect: int, state, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (4, 5, [0, 0, -1, 0, 0, 0])
    return numeric_instruction(instruction_info, condition, character, special_effect, state, target_comparison_type, target_count)

def IfCharacterHasSpecialEffect(condition: int, character: CharacterInt, special_effect, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterSpecialEffectState(condition, character, special_effect, True, target_comparison_type, target_count)

def IfCharacterDoesNotHaveSpecialEffect(condition: int, character: CharacterInt, special_effect, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterSpecialEffectState(condition, character, special_effect, False, target_comparison_type, target_count)

def IfPlayerHasSpecialEffect(condition: int, special_effect, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterSpecialEffectState(condition, PLAYER, special_effect, True, target_comparison_type, target_count)

def IfPlayerDoesNotHaveSpecialEffect(condition: int, special_effect, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterSpecialEffectState(condition, PLAYER, special_effect, False, target_comparison_type, target_count)


def IfCharacterBackreadState(condition: int, character: CharacterInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (4, 7)
    return numeric_instruction(instruction_info, condition, character, state, target_comparison_type, target_count)

def IfCharacterBackreadEnabled(condition: int, character: CharacterInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterBackreadState(condition, character, True, target_comparison_type, target_count)

def IfCharacterBackreadDisabled(condition: int, character: CharacterInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterBackreadState(condition, character, False, target_comparison_type, target_count)


def IfTAEEventState(condition: int, character: CharacterInt, tae_event_id: int, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (4, 8, [0, 0, -1, 0, 0, 0])
    return numeric_instruction(instruction_info, condition, character, tae_event_id, state, target_comparison_type, target_count)

def IfHasTAEEvent(condition: int, character: CharacterInt, tae_event_id: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfTAEEventState(condition, character, tae_event_id, True, target_comparison_type, target_count)

def IfDoesNotHaveTAEEvent(condition: int, character: CharacterInt, tae_event_id: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfTAEEventState(condition, character, tae_event_id, False, target_comparison_type, target_count)


def IfHasAIStatus(condition: int, character: CharacterInt, ai_status: AIStatusType, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (4, 9)
    return numeric_instruction(instruction_info, condition, character, ai_status, target_comparison_type, target_count)


def IfHealthValueComparison(condition: int, character: CharacterInt, comparison_type: ComparisonType, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (4, 14)
    return numeric_instruction(instruction_info, condition, character, comparison_type, value, target_comparison_type, target_count)

def IfHealthValueEqual(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthValueComparison(condition, character, ComparisonType.Equal, value, target_comparison_type, target_count)

def IfHealthValueNotEqual(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthValueComparison(condition, character, ComparisonType.NotEqual, value, target_comparison_type, target_count)

def IfHealthValueGreaterThan(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthValueComparison(condition, character, ComparisonType.GreaterThan, value, target_comparison_type, target_count)

def IfHealthValueLessThan(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthValueComparison(condition, character, ComparisonType.LessThan, value, target_comparison_type, target_count)

def IfHealthValueGreaterThanOrEqual(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthValueComparison(condition, character, ComparisonType.GreaterThanOrEqual, value, target_comparison_type, target_count)

def IfHealthValueLessThanOrEqual(condition: int, character: CharacterInt, value, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfHealthValueComparison(condition, character, ComparisonType.LessThanOrEqual, value, target_comparison_type, target_count)


def IfCharacterDrawGroupState(condition: int, character: CharacterInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    """ Tests if character's draw group is currently active or inactive. """
    instruction_info = (4, 15, [0, 0, 0, 0, 0])
    return numeric_instruction(instruction_info, condition, character, state, target_comparison_type, target_count)

def IfCharacterDrawGroupActive(condition: int, character: CharacterInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterDrawGroupState(condition, character, True, target_comparison_type, target_count)

def IfCharacterDrawGroupInactive(condition: int, character: CharacterInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfCharacterDrawGroupState(condition, character, False, target_comparison_type, target_count)
...
def IfCharacterInvadeType(condition: int, character: CharacterInt, invade_type: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    """ 'invade_type' has an unknown type in the EMEDF. Probably refers to the invader's covenant. """
    instruction_info = (4, 27)
    return numeric_instruction(instruction_info, condition, character, invade_type, target_comparison_type, target_count)
...
def IfObjectDestructionState(condition: int, obj: ObjectInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (5, 0)
    return numeric_instruction(instruction_info, condition, state, obj, target_comparison_type, target_count)

def IfObjectDestroyed(condition: int, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfObjectDestructionState(condition, obj, True, target_comparison_type, target_count)

def IfObjectNotDestroyed(condition: int, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfObjectDestructionState(condition, obj, False, target_comparison_type, target_count)


def IfObjectBurnState(condition: int, obj: ObjectInt, comparison_type: ComparisonType, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    ...
    return numeric_instruction(instruction_info, condition, obj, comparison_type, state, target_comparison_type, target_count)


def IfObjectBackreadState(condition: int, obj: ObjectInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    instruction_info = (5, 10)
    return numeric_instruction(instruction_info, condition, obj, state, target_comparison_type, target_count)

def IfObjectBackreadEnabled(condition: int, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfObjectBackreadState(condition, obj, True, target_comparison_type, target_count)

def IfObjectBackreadDisabled(condition: int, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfObjectBackreadState(condition, obj, False, target_comparison_type, target_count)


def IfObjectBackreadState_Alternate(condition: int, obj: ObjectInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
...
def IfObjectBackreadEnabled_Alternate(condition: int, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfObjectBackreadState_Alternate(condition, obj, True, target_comparison_type, target_count)

def IfObjectBackreadDisabled_Alternate(condition: int, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return IfObjectBackreadState_Alternate(condition, obj, False, target_comparison_type, target_count)
...
def GotoIfCharacterSpecialEffectState(label: Label, character: CharacterInt, special_effect: int, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    """ Note that 'target_count' is now an integer again... """
    instruction_info = (1003, 11)
    return numeric_instruction(instruction_info, label, character, special_effect, state, target_comparison_type, target_count)

def GotoIfCharacterHasSpecialEffect(label: Label, character: CharacterInt, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return GotoIfCharacterSpecialEffectState(label, character, special_effect, True, target_comparison_type, target_count)

def GotoIfCharacterDoesNotHaveSpecialEffect(label: Label, character: CharacterInt, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return GotoIfCharacterSpecialEffectState(label, character, special_effect, False, target_comparison_type, target_count)
...
def TerminateIfCharacterSpecialEffectState(event_end_type: EventEndType, character: CharacterInt, special_effect: int, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    instruction_info = (1003, 111)
    return numeric_instruction(instruction_info, event_end_type, character, special_effect, state, target_comparison_type, target_count)

def EndIfCharacterSpecialEffectState(character: CharacterInt, special_effect: int, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.End, character, special_effect, state, target_comparison_type, target_count)

def EndIfCharacterHasSpecialEffect(character: CharacterInt, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.End, character, special_effect, True, target_comparison_type, target_count)

def EndIfCharacterDoesNotHaveSpecialEffect(character: CharacterInt, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.End, character, special_effect, False, target_comparison_type, target_count)

def RestartIfCharacterSpecialEffectState(character: CharacterInt, special_effect: int, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.Restart, character, special_effect, state, target_comparison_type, target_count)

def RestartIfCharacterHasSpecialEffect(character: CharacterInt, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.Restart, character, special_effect, True, target_comparison_type, target_count)

def RestartIfCharacterDoesNotHaveSpecialEffect(character: CharacterInt, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.Restart, character, special_effect, False, target_comparison_type, target_count)

def EndIfPlayerHasSpecialEffect(special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.End, PLAYER, special_effect, True, target_comparison_type, target_count)

def EndIfPlayerDoesNotHaveSpecialEffect(special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.End, PLAYER, special_effect, False, target_comparison_type, target_count)

def RestartIfPlayerHasSpecialEffect(special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.Restart, PLAYER, special_effect, True, target_comparison_type, target_count)

def RestartIfPlayerDoesNotHaveSpecialEffect(special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return TerminateIfCharacterSpecialEffectState(EventEndType.Restart, PLAYER, special_effect, False, target_comparison_type, target_count)

def SkipLinesIfCharacterSpecialEffectState(line_count: int, character: CharacterInt, special_effect: int, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    instruction_info = (1003, 112, [0, 0, -1, 0, 0, 0])
    return numeric_instruction(instruction_info, line_count, character, special_effect, state, target_comparison_type, target_count)

def SkipLinesIfCharacterHasSpecialEffect(line_count: int, character: CharacterInt, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return SkipLinesIfCharacterSpecialEffectState(line_count, character, special_effect, True, target_comparison_type, target_count)

def SkipLinesIfCharacterDoesNotHaveSpecialEffect(line_count: int, character: CharacterInt, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return SkipLinesIfCharacterSpecialEffectState(line_count, character, special_effect, False, target_comparison_type, target_count)

def SkipLinesIfPlayerHasSpecialEffect(line_count: int, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return SkipLinesIfCharacterSpecialEffectState(line_count, PLAYER, special_effect, True, target_comparison_type, target_count)

def SkipLinesIfPlayerDoesNotHaveSpecialEffect(line_count: int, special_effect: int, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: int = 1):
    return SkipLinesIfCharacterSpecialEffectState(line_count, PLAYER, special_effect, False, target_comparison_type, target_count)


def GotoIfCharacterRegionState(label: Label, state: bool, character: CharacterInt, region: RegionInt,
                               min_target_count: int = 1):
...
def TerminateIfCharacterRegionState(event_end_type: EventEndType, state: bool, character: CharacterInt,
                                    region: RegionInt, min_target_count: int = 1):
...
def SkipLinesIfCharacterRegionState(line_count: int, state: bool, character: CharacterInt, region: RegionInt,
                                    min_target_count: int = 1):
...
def SkipLinesIfObjectDestructionState(line_count, obj: ObjectInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
...
def SkipLinesIfObjectDestroyed(line_count, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return SkipLinesIfObjectDestructionState(line_count, obj, True, target_comparison_type, target_count)

def SkipLinesIfObjectNotDestroyed(line_count, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return SkipLinesIfObjectDestructionState(line_count, obj, False, target_comparison_type, target_count)


def TerminateIfObjectDestructionState(event_end_type: EventEndType, obj: ObjectInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
...
def EndIfObjectDestroyed(obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return TerminateIfObjectDestructionState(EventEndType.End, obj, True, target_comparison_type, target_count)

def EndIfObjectNotDestroyed(obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return TerminateIfObjectDestructionState(EventEndType.End, obj, False, target_comparison_type, target_count)

def RestartIfObjectDestroyed(obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return TerminateIfObjectDestructionState(EventEndType.Restart, obj, True, target_comparison_type, target_count)

def RestartIfObjectNotDestroyed(obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return TerminateIfObjectDestructionState(EventEndType.Restart, obj, False, target_comparison_type, target_count)


def GotoIfObjectDestructionState(label: Label, obj: ObjectInt, state: bool, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    """ Note change in argument order. """
...
def GotoIfObjectDestroyed(label: Label, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return GotoIfObjectDestructionState(label, obj, True, target_comparison_type, target_count)

def GotoIfObjectNotDestroyed(label: Label, obj: ObjectInt, target_comparison_type: ComparisonType = ComparisonType.Equal, target_count: float = 1.0):
    return GotoIfObjectDestructionState(label, obj, False, target_comparison_type, target_count)
...
def PlayCutsceneAndMovePlayerAndSetMapCeremony(cutscene: int, cutscene_type: CutsceneType, ceremony_id: int,
                                               unknown: int, region: RegionInt, game_map: GameMap,
                                               player_id: int):
    ...
    area_id, block_id = tuple(game_map)
    return numeric_instruction(instruction_info, cutscene, cutscene_type, ceremony_id, unknown, region, area_id, block_id,
                               player_id)
...
def PlayCutsceneAndMovePlayer_WithUnknowns(cutscene: int, cutscene_type: CutsceneType, region: RegionInt,
                                           game_map: GameMap, player_id: int, unknown1: int, unknown2: int):
...
def PlayCutsceneAndMovePlayer_WithSecondRegion(cutscene: int, cutscene_type: CutsceneType, region: RegionInt,
                                               game_map: GameMap, player_id: int, other_region: RegionInt):
...
def InitializeCrowTrade(item_type: ItemType, item_id: ItemInt, item_lot_id: ItemLotInt, trade_completed_flag: FlagInt,
                        crow_response_flag: FlagInt):
    instruction_info = (2003, 66)
    return numeric_instruction(instruction_info, item_type, item_id, item_lot_id, trade_completed_flag, crow_response_flag)
...
def SetDecoratedBossHealthBarState(state: bool, character: CharacterInt, slot: int, name: TextInt, decoration: int):
...
def EnableDecoratedBossHealthBar(character: CharacterInt, slot: int, name: TextInt, decoration: int):
    return SetDecoratedBossHealthBarState(True, character, slot, name, decoration)

def DisableDecoratedBossHealthBar(character: CharacterInt, slot: int, name: TextInt, decoration: int):
    return SetDecoratedBossHealthBarState(False, character, slot, name, decoration)


def PlaceNPCSummonSign_WithoutEmber(sign_type: SummonSignType, character: CharacterInt, region: RegionInt,
                                    summon_flag: FlagInt, dismissal_flag: FlagInt):
...
def DisplayDialogAndSetFlags(message: TextInt, button_type: ButtonType, number_buttons: NumberButtons,
                             anchor_entity: MapEntityInt, display_distance: float, left_flag: FlagInt,
                             right_flag: FlagInt, cancel_flag: FlagInt):
...
def DisplayAreaWelcomeMessage(message: TextInt):
    """ Not sure what this looks like exactly. """
    instruction_info = (2007, 11)
    return numeric_instruction(instruction_info, message)


def DisplayHollowArenaMessage(message: TextInt, unknown: int, pad_enabled: bool):
...
def ActivateHitboxAndCreateNavimesh(hitbox: HitboxInt, state: bool):
