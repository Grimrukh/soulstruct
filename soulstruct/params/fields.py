...
def _behavior_ref_id(behavior_param_entry):
    if behavior_param_entry["refType"] == BEHAVIOR_REF_TYPE.Default:
        return "Attack", True, Params.Attacks, "Attack ID triggered by behavior."
    elif behavior_param_entry["refType"] == BEHAVIOR_REF_TYPE.Bullet:
        return "Bullet", True, Params.Bullets, "Bullet ID triggered by behavior."
        ...
        return "UnknownReferenceID", True, int, "Could not determine reference ID type (usually Attack or Bullet)."
...
def _good_ref_id(goods_param_entry):
    if goods_param_entry["refCategory"] == BEHAVIOR_REF_TYPE.Default:
        ...
        return "SpecialEffect", True, Params.SpecialEffects, "Special effect triggered (on self) by using good."
    else:
        return (
            "UnknownReferenceID",
            True,
...
def _spells_ref_id(spells_param_entry):
    if spells_param_entry["refCategory"] == BEHAVIOR_REF_TYPE.Default:
        ...
        return "SpecialEffect", True, Params.SpecialEffects, "Special effect triggered (on self) by casting spell."
    else:
        return (
            "UnknownReferenceID",
            True,
...
def _obj_act_success_condition(condition_field_name, obj_act_param_entry):
    ...
    if obj_act_param_entry[condition_field_name] == OBJACT_SP_QUALIFIED_TYPE.NoCondition:
        return f"NoCondition{n}", True, int, f"No condition type selected."
...
def _item_lot_item_id(item_lot_slot: int, item_lot_param_entry):
    ...
    if item_type == ITEMLOT_ITEMCATEGORY.Weapon:
        return f"ItemSlot{item_lot_slot}", True, Params.Weapons, f"Item slot {item_lot_slot} (Weapon)."
    elif item_type == ITEMLOT_ITEMCATEGORY.Armor:
        return f"ItemSlot{item_lot_slot}", True, Params.Armor, f"Item slot {item_lot_slot} (Armor)."
    elif item_type == ITEMLOT_ITEMCATEGORY.Ring:
        return f"ItemSlot{item_lot_slot}", True, Params.Rings, f"Item slot {item_lot_slot} (Ring)."
    elif item_type == ITEMLOT_ITEMCATEGORY.Good:
        return f"ItemSlot{item_lot_slot}", True, Params.Goods, f"Item slot {item_lot_slot} (Good)."
    else:
        return f"ItemSlot{item_lot_slot}", True, int, f"Item slot {item_lot_slot} (unknown item type {item_type})."


def _shop_item_id(shops_param_entry):
    item_type = shops_param_entry[f"equipType"]
    if item_type == SHOP_LINEUP_EQUIPTYPE.Weapon:
        return f"Weapon", True, Params.Weapons, f"Weapon to be listed in shop menu."
    if item_type == SHOP_LINEUP_EQUIPTYPE.Armor:
        return f"Armor", True, Params.Armor, f"Armor to be listed in shop menu."
    if item_type == SHOP_LINEUP_EQUIPTYPE.Ring:
        return f"Ring", True, Params.Rings, f"Ring to be listed in shop menu."
    if item_type == SHOP_LINEUP_EQUIPTYPE.Good:
        return f"Good", True, Params.Goods, f"Good to be listed in shop menu."
    if item_type == SHOP_LINEUP_EQUIPTYPE.Spell:
        return f"Spell", True, Params.Spells, f"Spell to be listed in attunement menu."
    else:
        return f"ItemID", True, int, f"Item to be listed in shop/attunement menu (unknown item type {item_type})."
...
GAME_PARAM_INFO = {
    ...
    "NPC_THINK_PARAM_ST": {
        ...
        "battleGoalID": ("BattleID", True, AI.Battle, "Battle goal ID used to look up battle Lua script."),
        "nearDist": ("NearDistance", True, float, "Distance considered to be close range by this NPC (for scripts)."),
        "midDist": ("MidDistance", True, float, "Distance considered to be medium range by this NPC (for scripts)."),
        ...
        "eye_dist": ("SightDistance", True, int, "Distance of NPC eyesight (in meters)."),
        "ear_dist": ("HearingDistance", True, int, "Distance of NPC hearing (in meters)."),
        "ear_soundcut_dist": (
            "HearingCutDistance",
            False,
            int,
        ...
        "nose_dist": ("SmellDistance", True, int, "Distance of NPC smell (auto-detect)."),
        "maxBackhomeDist": (
            "MaxRetreatDistance",
            True,
            int,
        ...
        "BattleStartDist": ("BattleStartDistance", True, int, "Target distance at which battle mode is triggered."),
        "callHelp_MyPeerId": (
            "HelpGroupID",
            True,
            int,
        ...
        "eye_angX": ("SightRangeHeight", True, int, "Angular width of sight field in degrees."),
        "eye_angY": ("SightRangeWidth", True, int, "Angular height of sight field in degrees."),
        "ear_angX": ("HearingRangeHeight", True, int, "Angular width of hearing field in degrees."),
        "ear_angY": ("HearingRangeWidth", True, int, "Angular height of hearing field in degrees."),
        ...
        "disablePathMove": (
            "IgnoreNavmesh",
            True,
            bool,  # NOTE: Not a boolean field, but used like one.
    ...
    "EQUIP_PARAM_PROTECTOR_ST": {
        "sortId": ("SortIndex", True, int, "Index for automatic inventory sorting."),
        "wanderingEquipId": ("GhostArmorReplacement", True, Params.Armor, "Replacement equipment for network ghosts."),
        "vagrantItemLotId": ("VagrantItemLot", True, Params.ItemLots, "DOC-TODO"),
        "vagrantBonusEneDropItemLotId": ("VagrantBonusEnemyDropItemLot", True, Params.ItemLots, "DOC-TODO"),
        "vagrantItemEneDropItemLotId": ("VagrantItemEnemyDropItemLot", True, Params.ItemLots, "DOC-TODO"),
        "fixPrice": (
            "RepairCost",
            True,
            int,
        ...
        "weight": ("Weight", True, float, "Weight of armor."),
        "residentSpEffectId": (
        ...
        "qwcId": ("QWCID", False, int, "Unused world tendency remnant."),
        "equipModelId": ("EquipmentModel", True, Model, "Model ID of armor."),
        "iconIdM": ("MaleIcon", True, Texture, "Icon of male variant of armor in inventory."),
        "iconIdF": ("FemaleIcon", True, Texture, "Icon of female variant of armor in inventory."),
        "knockBack": (
        ...
        "durabilityMax": ("MaxDurability", True, int, "Maximum durability of armor."),
        "saDurability": ("Poise", True, int, "Amount of poise added when wearing armor."),
        ...
        "defenseMagic": ("MagicDefense", True, int, "Added defense against magic attack damage."),
        "defenseFire": ("FireDefense", True, int, "Added defense against fire attack damage."),
        ...
        "resistPoison": ("PoisonResistance", True, int, "Poison resistance added by armor."),
        "resistDisease": ("ToxicResistance", True, int, "Toxic resistance added by armor."),
        "resistBlood": ("BleedResistance", True, int, "Bleed resistance added by armor."),
        "resistCurse": ("CurseResistance", True, int, "Curse resistance added by armor."),
        "reinforceTypeId": (
            "ArmorUpgradeID",
            True,
            Params.ArmorUpgrades,
        ...
        "knockbackParamId": ("KnockbackID", False, Params.Knockback, "Knockback entry. Always 1."),
        "flickDamageCutRate": (
        ...
        "protectorCategory": ("ArmorType", True, PROTECTOR_CATEGORY, "Type of armor (equip slot)."),
        "defenseMaterial": (
            "SoundEffectOnHit",
            True,
            WEP_MATERIAL_DEF,
        ...
        "partsDmgType": ("PartsDamageType", False, ATK_PARAM_PARTSDMGTYPE, "Always zero."),
        ...
        "headEquip:1": ("EquippedToHead", True, bool, "This armor is equipped to the head."),
        "bodyEquip:1": ("EquippedToBody", True, bool, "This armor is equipped to the body."),
        "armEquip:1": ("EquippedToHands", True, bool, "This armor is equipped to the hands."),
        "legEquip:1": ("EquippedToLegs", True, bool, "This armor is equipped to the legs."),
        "useFaceScale:1": (
            "UseFaceScale",
            False,
            bool,
        ...
        "invisibleFlag00:1": ("HideFlag0", False, bool, "Hide part of the character model: (unknown)"),
        ...
        "invisibleFlag07:1": ("HideFlag7", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag08:1": ("HideFlag8", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag09:1": ("HideFlag9", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag10:1": ("HideFlag10", False, bool, "Hide part of the character model: (collar)"),
        "invisibleFlag11:1": ("HideFlag11", False, bool, "Hide part of the character model: (around collar)"),
        "invisibleFlag12:1": ("HideFlag12", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag13:1": ("HideFlag13", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag14:1": ("HideFlag14", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag15:1": ("HideFlag15", False, bool, "Hide part of the character model: (hood hem)"),
        "invisibleFlag16:1": ("HideFlag16", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag17:1": ("HideFlag17", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag18:1": ("HideFlag18", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag19:1": ("HideFlag19", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag20:1": ("HideFlag20", False, bool, "Hide part of the character model: (sleeve A)"),
        "invisibleFlag21:1": ("HideFlag21", False, bool, "Hide part of the character model: (sleeve B)"),
        "invisibleFlag22:1": ("HideFlag22", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag23:1": ("HideFlag23", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag24:1": ("HideFlag24", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag25:1": ("HideFlag25", False, bool, "Hide part of the character model: (arm)"),
        "invisibleFlag26:1": ("HideFlag26", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag27:1": ("HideFlag27", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag28:1": ("HideFlag28", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag29:1": ("HideFlag29", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag30:1": ("HideFlag30", False, bool, "Hide part of the character model: (belt)"),
        "invisibleFlag31:1": ("HideFlag31", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag32:1": ("HideFlag32", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag33:1": ("HideFlag33", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag34:1": ("HideFlag34", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag35:1": ("HideFlag35", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag36:1": ("HideFlag36", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag37:1": ("HideFlag37", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag38:1": ("HideFlag38", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag39:1": ("HideFlag39", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag40:1": ("HideFlag40", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag41:1": ("HideFlag41", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag42:1": ("HideFlag42", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag43:1": ("HideFlag43", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag44:1": ("HideFlag44", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag45:1": ("HideFlag45", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag46:1": ("HideFlag46", False, bool, "Hide part of the character model: (unknown)"),
        "invisibleFlag47:1": ("HideFlag47", False, bool, "Hide part of the character model: (unknown)"),
        ...
        "simpleModelForDlc:1": ("SimpleDLCModelExists", False, bool, "Unknown; always set to False."),
        "pad_0[1]": ("Pad0", False, "<Pad:1>", "Null padding."),
        "oldSortId": ("OldSortIndex", False, int, "Sorting index for an obsolete build of the game. No effect."),
        "pad_1[6]": ("Pad1", False, "<Pad:6>", "Null padding."),
    },
    "REINFORCE_PARAM_PROTECTOR_ST": {
        ...
        "magicDefRate": ("MagicDefenseMultiplier", True, float, "Multiplier for magic defense at this upgrade level."),
        "fireDefRate": ("FireDefenseMultiplier", True, float, "Multiplier for fire defense at this upgrade level."),
        "thunderDefRate": (
        ...
        "slashDefRate": ("SlashDefenseMultiplier", True, float, "Multiplier for slash defense at this upgrade level."),
        "blowDefRate": ("StrikeDefenseMultiplier", True, float, "Multiplier for strike defense at this upgrade level."),
        "thrustDefRate": (
    ...
    "BULLET_PARAM_ST": {
        ...
        "sfxId_Bullet": ("ProjectileFX", True, int, "Visual effect ID for bullet projectile."),
        "sfxId_Hit": ("ImpactFX", True, int, "Visual effect ID for bullet impact."),
        "sfxId_Flick": (
            "FlickFX",
            True,
            int,
        ...
        "gravityInRange": ("GravityBeforeAttenuation", True, float, "Downward acceleration of bullet. Rarely used."),
        "gravityOutRange": (
        ...
        "initVellocity": ("InitialSpeed", True, float, "Initial speed of bullet."),
        "accelInRange": (
        ...
        "maxVellocity": ("MaxSpeed", True, float, "Maximum speed of bullet, regardless of acceleration."),
        "minVellocity": ("MinSpeed", True, float, "Minimum speed of bullet, regardless of acceleration."),
        ...
        "hitRadius": ("InitialHitRadius", True, float, "Initial hit radius of bullet projectile."),
        "hitRadiusMax": (
            "FinalHitRadius",
            True,
            float,
        ...
        "dmgHitRecordLifeTime": (
            ...
            True,
            float,
            "Duration of bullet impact hitbox. A value of zero means it is disabled immediately " "after first impact.",
        ),
        "externalForce": (
            "ExternalForce",
            True,
        ...
        "numShoot": ("BulletCount", True, int, "Number of bullets emitted at once."),
        "homingAngle": (
        ...
        "fireDamageDamp": ("FireDamageDamp", True, int, "Percentage reduction in fire damage per second."),
        "thunderDamageDamp": ("LightningDamageDamp", True, int, "Percentage reduction in lightning damage per second."),
        "staminaDamp": ("StaminaDamp", False, int, "Percentage reduction in stamina damage per second."),
        ...
        "spAttribute": ("ElementAttribute", True, ATKPARAM_SPATTR_TYPE, "Element attached to bullet hit."),
        ...
        "Material_Size": ("MaterialSize", False, ATK_SIZE, "'Size' of attack. Never used.'"),
        "launchConditionType": (
            "LaunchConditionType",
            True,
        ...
        "FollowType:3": ("FollowType", True, BULLET_FOLLOW_TYPE, "Follow type."),
        "EmittePosType:3": (
            "OriginType",
            True,
    ...
    "LOCK_CAM_PARAM_ST": {
        ...
        "pad[8]": ("Pad1", False, "<Pad:8>", "Null padding."),
    },
    "ATK_PARAM_ST": {
        "hit0_Radius": ("Hitbox0Radius", True, float, "Radius of sphere/capsule hitbox (slot 0)."),
        "hit1_Radius": ("Hitbox1Radius", True, float, "Radius of sphere/capsule hitbox (slot 1)."),
        "hit2_Radius": ("Hitbox2Radius", True, float, "Radius of sphere/capsule hitbox (slot 2)."),
        "hit3_Radius": ("Hitbox3Radius", True, float, "Radius of sphere/capsule hitbox (slot 3)."),
        "knockbackDist": ("KnockbackDistance", True, float, "Knockback distance of attack."),
        "hitStopTime": (
            "HitStopTime",
            True,
            float,
        ...
        "blowingCorrection": ("BlowOffCorrection", False, int, "Unknown. Never used."),
        "atkPhysCorrection": (
            "PhysicalAttackPowerPercentage",
            True,
            int,  # TODO: Player Attacks only.
        ...
        "atkPhys": (
            "PhysicalAttackPower",
            True,
            int,  # TODO: Non Player Attacks only.
        ...
        "atkThrowEscape": (
            "AttackPowerDuringThrows",
            False,
            int,  # TODO: Non Player Attacks only.
            "Absolute attack power of attack. Never used.",
        ),
        "atkObj": ("ObjectDamage", True, int, "Amount of damage dealt to objects by this attack."),
        "guardStaminaCutRate": (
        ...
        "throwTypeId": (
            "ThrowID",
            True,
            int,  # TODO: Link to Throws using the ThrowID field rather than by entry ID.
        ...
        "mapHitType": ("MapHitType", True, ATK_PARAM_MAP_HIT, "Determines how this attack interacts with the map."),
        "guardCutCancelRate": (
        ...
        "atkAttribute": ("AttackAttribute", True, ATKPARAM_ATKATTR_TYPE, "Type of physical damage done by attack."),
        "spAttribute": (
            "ElementAttribute",
            True,
        ...
        "isGhostAtk:1": (
            "IsAttackByGhost",
            True,
            bool,  # TODO: Non Player Attacks only.
        ...
        "pad[1]": ("Pad1", False, "<Pad:1>", "Null padding."),
    },
    "BEHAVIOR_PARAM_ST": {
        ...
        "sfxVariationId": ("FXVariationID", True, int, "Visual effect ID."),
        "stamina": ("StaminaCost", True, int, "Stamina cost of behavior."),
        "mp": ("DurabilityCost", True, int, "Weapon/shield durability cost of behavior."),
        "category": (
            "Category",
            True,
            BEHAVIOR_CATEGORY,
        ...
        "heroPoint": ("HumanityCost", False, int, "Humanity cost of behavior. Never used."),
        "pad1[2]": ("Pad2", False, "<Pad:2>", "Null padding."),
    },
    "CHARACTER_INIT_PARAM": {
        "baseRec_mp": ("BaseRecMP", False, float, "Unknown."),
        "baseRec_sp": ("BaseRecSP", False, float, "Unknown."),
        "red_Falldam": ("RedFallDamage", False, float, "Unknown."),
        ...
        "equip_Subwep_Left": ("LeftHandWeapon2", True, Params.Weapons, "Second weapon/shield equipped in left hand."),
        ...
        ),  # TODO: points to Goods or Spells?
        "equip_Spell_02": ("SpellSlot2", True, Params.Spells, "Second spell equipped."),
        "equip_Spell_03": ("SpellSlot3", True, Params.Spells, "Third spell equipped."),
        "equip_Spell_04": ("SpellSlot4", True, Params.Spells, "Fourth spell equipped."),
        "equip_Spell_05": ("SpellSlot5", True, Params.Spells, "Fifth spell equipped."),
        "equip_Spell_06": ("SpellSlot6", True, Params.Spells, "Sixth spell equipped."),
        "equip_Spell_07": ("SpellSlot7", True, Params.Spells, "Seventh spell equipped."),
        "item_01": ("GoodSlot1", True, Params.Goods, "Good (item) equipped in slot 1."),
        "item_02": ("GoodSlot2", True, Params.Goods, "Good (item) equipped in slot 2."),
        "item_03": ("GoodSlot3", True, Params.Goods, "Good (item) equipped in slot 3."),
        "item_04": ("GoodSlot4", True, Params.Goods, "Good (item) equipped in slot 4."),
        "item_05": ("GoodSlot5", True, Params.Goods, "Good (item) equipped in slot 5."),
        "item_06": ("GoodSlot6", True, Params.Goods, "Good (item) equipped in slot 6."),
        "item_07": ("GoodSlot7", True, Params.Goods, "Good (item) equipped in slot 7."),
        "item_08": ("GoodSlot8", True, Params.Goods, "Good (item) equipped in slot 8."),
        "item_09": ("GoodSlot9", True, Params.Goods, "Good (item) equipped in slot 9."),
        "item_10": ("GoodSlot10", True, Params.Goods, "Good (item) equipped in slot 10."),
        "npcPlayerFaceGenId": ("FaceID", True, Params.Faces, "Face parameter ID (NPCs only)."),
        "npcPlayerThinkId": ("DefaultAI", True, Params.AI, "Default AI (NPCs only)."),
    ...
    "FACE_PARAM_ST": {
        "faceGeoData00": ("GeometryData00", True, int, "Geometry data point 0."),
        "faceGeoData01": ("GeometryData01", True, int, "Geometry data point 1."),
        "faceGeoData02": ("GeometryData02", True, int, "Geometry data point 2."),
        "faceGeoData03": ("GeometryData03", True, int, "Geometry data point 3."),
        "faceGeoData04": ("GeometryData04", True, int, "Geometry data point 4."),
        "faceGeoData05": ("GeometryData05", True, int, "Geometry data point 5."),
        "faceGeoData06": ("GeometryData06", True, int, "Geometry data point 6."),
        "faceGeoData07": ("GeometryData07", True, int, "Geometry data point 7."),
        "faceGeoData08": ("GeometryData08", True, int, "Geometry data point 8."),
        "faceGeoData09": ("GeometryData09", True, int, "Geometry data point 9."),
        "faceGeoData10": ("GeometryData10", True, int, "Geometry data point 10."),
        "faceGeoData11": ("GeometryData11", True, int, "Geometry data point 11."),
        "faceGeoData12": ("GeometryData12", True, int, "Geometry data point 12."),
        "faceGeoData13": ("GeometryData13", True, int, "Geometry data point 13."),
        "faceGeoData14": ("GeometryData14", True, int, "Geometry data point 14."),
        "faceGeoData15": ("GeometryData15", True, int, "Geometry data point 15."),
        "faceGeoData16": ("GeometryData16", True, int, "Geometry data point 16."),
        "faceGeoData17": ("GeometryData17", True, int, "Geometry data point 17."),
        "faceGeoData18": ("GeometryData18", True, int, "Geometry data point 18."),
        "faceGeoData19": ("GeometryData19", True, int, "Geometry data point 19."),
        "faceGeoData20": ("GeometryData20", True, int, "Geometry data point 20."),
        "faceGeoData21": ("GeometryData21", True, int, "Geometry data point 21."),
        "faceGeoData22": ("GeometryData22", True, int, "Geometry data point 22."),
        "faceGeoData23": ("GeometryData23", True, int, "Geometry data point 23."),
        "faceGeoData24": ("GeometryData24", True, int, "Geometry data point 24."),
        "faceGeoData25": ("GeometryData25", True, int, "Geometry data point 25."),
        "faceGeoData26": ("GeometryData26", True, int, "Geometry data point 26."),
        "faceGeoData27": ("GeometryData27", True, int, "Geometry data point 27."),
        "faceGeoData28": ("GeometryData28", True, int, "Geometry data point 28."),
        "faceGeoData29": ("GeometryData29", True, int, "Geometry data point 29."),
        "faceGeoData30": ("GeometryData30", True, int, "Geometry data point 30."),
        "faceGeoData31": ("GeometryData31", True, int, "Geometry data point 31."),
        "faceGeoData32": ("GeometryData32", True, int, "Geometry data point 32."),
        "faceGeoData33": ("GeometryData33", True, int, "Geometry data point 33."),
        "faceGeoData34": ("GeometryData34", True, int, "Geometry data point 34."),
        "faceGeoData35": ("GeometryData35", True, int, "Geometry data point 35."),
        "faceGeoData36": ("GeometryData36", True, int, "Geometry data point 36."),
        "faceGeoData37": ("GeometryData37", True, int, "Geometry data point 37."),
        "faceGeoData38": ("GeometryData38", True, int, "Geometry data point 38."),
        "faceGeoData39": ("GeometryData39", True, int, "Geometry data point 39."),
        "faceGeoData40": ("GeometryData40", True, int, "Geometry data point 40."),
        "faceGeoData41": ("GeometryData41", True, int, "Geometry data point 41."),
        "faceGeoData42": ("GeometryData42", True, int, "Geometry data point 42."),
        "faceGeoData43": ("GeometryData43", True, int, "Geometry data point 43."),
        "faceGeoData44": ("GeometryData44", True, int, "Geometry data point 44."),
        "faceGeoData45": ("GeometryData45", True, int, "Geometry data point 45."),
        "faceGeoData46": ("GeometryData46", True, int, "Geometry data point 46."),
        "faceGeoData47": ("GeometryData47", True, int, "Geometry data point 47."),
        "faceGeoData48": ("GeometryData48", True, int, "Geometry data point 48."),
        "faceGeoData49": ("GeometryData49", True, int, "Geometry data point 49."),
        "faceTexData00": ("TextureData00", True, int, "Texture data point 0."),
        "faceTexData01": ("TextureData01", True, int, "Texture data point 1."),
        "faceTexData02": ("TextureData02", True, int, "Texture data point 2."),
        "faceTexData03": ("TextureData03", True, int, "Texture data point 3."),
        "faceTexData04": ("TextureData04", True, int, "Texture data point 4."),
        "faceTexData05": ("TextureData05", True, int, "Texture data point 5."),
        "faceTexData06": ("TextureData06", True, int, "Texture data point 6."),
        "faceTexData07": ("TextureData07", True, int, "Texture data point 7."),
        "faceTexData08": ("TextureData08", True, int, "Texture data point 8."),
        "faceTexData09": ("TextureData09", True, int, "Texture data point 9."),
        "faceTexData10": ("TextureData10", True, int, "Texture data point 10."),
        "faceTexData11": ("TextureData11", True, int, "Texture data point 11."),
        "faceTexData12": ("TextureData12", True, int, "Texture data point 12."),
        "faceTexData13": ("TextureData13", True, int, "Texture data point 13."),
        "faceTexData14": ("TextureData14", True, int, "Texture data point 14."),
        "faceTexData15": ("TextureData15", True, int, "Texture data point 15."),
        "faceTexData16": ("TextureData16", True, int, "Texture data point 16."),
        "faceTexData17": ("TextureData17", True, int, "Texture data point 17."),
        "faceTexData18": ("TextureData18", True, int, "Texture data point 18."),
        "faceTexData19": ("TextureData19", True, int, "Texture data point 19."),
        "faceTexData20": ("TextureData20", True, int, "Texture data point 20."),
        "faceTexData21": ("TextureData21", True, int, "Texture data point 21."),
        "faceTexData22": ("TextureData22", True, int, "Texture data point 22."),
        "faceTexData23": ("TextureData23", True, int, "Texture data point 23."),
        "faceTexData24": ("TextureData24", True, int, "Texture data point 24."),
        "faceTexData25": ("TextureData25", True, int, "Texture data point 25."),
        "faceTexData26": ("TextureData26", True, int, "Texture data point 26."),
        "faceTexData27": ("TextureData27", True, int, "Texture data point 27."),
        "faceTexData28": ("TextureData28", True, int, "Texture data point 28."),
        "faceTexData29": ("TextureData29", True, int, "Texture data point 29."),
        "faceTexData30": ("TextureData30", True, int, "Texture data point 30."),
        "faceTexData31": ("TextureData31", True, int, "Texture data point 31."),
        "faceTexData32": ("TextureData32", True, int, "Texture data point 32."),
        "faceTexData33": ("TextureData33", True, int, "Texture data point 33."),
        "faceTexData34": ("TextureData34", True, int, "Texture data point 34."),
        "faceTexData35": ("TextureData35", True, int, "Texture data point 35."),
        "faceTexData36": ("TextureData36", True, int, "Texture data point 36."),
        "faceTexData37": ("TextureData37", True, int, "Texture data point 37."),
        "faceTexData38": ("TextureData38", True, int, "Texture data point 38."),
        "faceTexData39": ("TextureData39", True, int, "Texture data point 39."),
        "faceTexData40": ("TextureData40", True, int, "Texture data point 40."),
        "faceTexData41": ("TextureData41", True, int, "Texture data point 41."),
        "faceTexData42": ("TextureData42", True, int, "Texture data point 42."),
        "faceTexData43": ("TextureData43", True, int, "Texture data point 43."),
        "faceTexData44": ("TextureData44", True, int, "Texture data point 44."),
        "faceTexData45": ("TextureData45", True, int, "Texture data point 45."),
        "faceTexData46": ("TextureData46", True, int, "Texture data point 46."),
        "faceTexData47": ("TextureData47", True, int, "Texture data point 47."),
        "faceTexData48": ("TextureData48", True, int, "Texture data point 48."),
        "faceTexData49": ("TextureData49", True, int, "Texture data point 49."),
        "hairStyle": ("HairStyle", True, FACE_PARAM_HAIRSTYLE_TYPE, "Hairstyle of face."),
        "hairColor_Base ": ("BaseHairColor", True, FACE_PARAM_HAIRCOLOR_TYPE, "Base hair color of face."),
        "hairColor_R ": ("HairColorRed", True, int, "Red channel of hair color (0 to 255)."),
        "hairColor_G ": ("HairColorGreen", True, int, "Greenchannel of hair color (0 to 255)."),
        "hairColor_B ": ("HairColorBlue", True, int, "Blue channel of hair color (0 to 255)."),
        "eyeColor_R": ("EyeColorRed", True, int, "Red channel of eye color (0 to 255)."),
        "eyeColor_G": ("EyeColorGreen", True, int, "Green channel of eye color (0 to 255)."),
        "eyeColor_B": ("EyeColorBlue", True, int, "Blue channel of eye color (0 to 255)."),
        "pad[20]": ("Pad1", False, "<Pad:20>", "Null padding."),
    },
    "EQUIP_PARAM_GOODS_ST": {
        ...
        "weight": ("Weight", False, float, "Weight of good. Never used in vanilla Dark Souls."),
        "basicPrice": ("BasicCost", False, int, "Unsure. Does not appear to be used."),
        "sellValue": (
            "FramptSellValue",
            True,
            int,
        ...
        "behaviorId": ("Behavior", False, Params.Behaviors, "Behavior triggered by good use. Never used."),
        "replaceItemId": (
            "GoodToReplace",
            True,
            Params.Goods,
        ...
        "iconId": ("GoodIcon", True, Texture, "Good icon texture ID."),
        "modelId": ("ModelID", False, Model, "Model of good. Never used."),
        "shopLv": (
            "ShopLevel",
            False,
            int,
        ...
        "maxNum": ("MaxHoldQuantity", True, int, "Maximum number of good that can be held at once."),
        "consumeHeroPoint": ("HumanityCost", False, int, "Humanity cost of using good. Always zero."),
        "overDexterity": (
            "OverDexterity",
            False,
            int,
        ...
        "goodsCategory": ("GoodCategory", False, GOODS_CATEGORY, "Never used. Only one value (0) used."),
        "goodsUseAnim": (
            "UseAnimation",
            True,
            GOODS_USE_ANIM,
        ...
        "isEquip:1": ("CanBeEquipped", True, bool, "Determines if this good can be equipped in a quick item slot."),
        "isConsume:1": (
            "ConsumedOnUse",
            True,
            bool,
        ...
        "isEstablishment:1": ("IsStationary", True, bool, "Unknown; need to look at usage."),
        "isOnlyOne:1": ("IsUnique", True, bool, "Determines if only one of this good exists in the game."),
        "isDrop:1": ("CanBeDropped", True, bool, "Determines if this item can be dropped."),
        "isDeposit:1": ("CanBeStored", True, bool, "Determines if good can be stored in Bottomless Box."),
        ...
        "isEnhance:1": ("IsUpgradeMaterial", True, bool, "Determines if this is an upgrade material."),
        "isFixItem:1": ("IsFixItem?", True, bool, "Probably True for Repair Powder, etc."),
        "disableMultiDropShare:1": (
            "DisableMultiplayerShare",
            True,
            bool,
        ...
        "pad[9]": ("Pad1", False, "<Pad:9>", "Null padding."),
        "vagrantItemLotId": ("VagrantItemLot", False, Params.ItemLots, "DOC-TODO"),
        "vagrantBonusEneDropItemLotId": ("VagrantBonusEnemyDropItemLot", False, Params.ItemLots, "DOC-TODO"),
        "vagrantItemEneDropItemLotId": ("VagrantItemEnemyDropItemLot", False, Params.ItemLots, "DOC-TODO"),
    },
    "HIT_MTRL_PARAM_ST": {
        "aiVolumeRate": (
        ...
        "pad0[3]": ("Pad1", False, "<Pad:3>", "Null padding."),
    },
    "ITEMLOT_PARAM_ST": {
        ...
        "lotItemCategory01": ("Item1Category", True, ITEMLOT_ITEMCATEGORY, "Type of item (slot 1)."),
        "lotItemId01": partial(_item_lot_item_id, 1),
        "lotItemNum01": ("Item1Count", True, int, "Count of item (slot 1)."),
        "lotItemBasePoint01": (
            "Item1ChancePoints",
            True,
            int,
        ...
        "lotItemCategory02": ("Item2Category", True, ITEMLOT_ITEMCATEGORY, "Type of item (slot 2)."),
        "lotItemId02": partial(_item_lot_item_id, 2),
        "lotItemNum02": ("Item2Count", True, int, "Count of item (slot 2)."),
        "lotItemBasePoint02": (
            "Item2ChancePoints",
            True,
            int,
        ...
        "lotItemCategory03": ("Item3Category", True, ITEMLOT_ITEMCATEGORY, "Type of item (slot 3)."),
        "lotItemId03": partial(_item_lot_item_id, 3),
        "lotItemNum03": ("Item3Count", True, int, "Count of item (slot 3)."),
        "lotItemBasePoint03": (
            "Item3ChancePoints",
            True,
            int,
        ...
        "lotItemCategory04": ("Item4Category", True, ITEMLOT_ITEMCATEGORY, "Type of item (slot 4)."),
        "lotItemId04": partial(_item_lot_item_id, 4),
        "lotItemNum04": ("Item4Count", True, int, "Count of item (slot 4)."),
        "lotItemBasePoint04": (
            "Item4ChancePoints",
            True,
            int,
        ...
        "lotItemCategory05": ("Item5Category", True, ITEMLOT_ITEMCATEGORY, "Type of item (slot 5)."),
        "lotItemId05": partial(_item_lot_item_id, 5),
        "lotItemNum05": ("Item5Count", True, int, "Count of item (slot 5)."),
        "lotItemBasePoint05": (
            "Item5ChancePoints",
            True,
            int,
        ...
        "lotItemCategory06": ("Item6Category", True, ITEMLOT_ITEMCATEGORY, "Type of item (slot 6)."),
        "lotItemId06": partial(_item_lot_item_id, 6),
        "lotItemNum06": ("Item6Count", True, int, "Count of item (slot 6)."),
        "lotItemBasePoint06": (
            "Item6ChancePoints",
            True,
            int,
        ...
        "lotItemCategory07": ("Item7Category", True, ITEMLOT_ITEMCATEGORY, "Type of item (slot 7)."),
        "lotItemId07": partial(_item_lot_item_id, 7),
        "lotItemNum07": ("Item7Count", True, int, "Count of item (slot 7)."),
        "lotItemBasePoint07": (
            "Item7ChancePoints",
            True,
            int,
        ...
        "lotItemCategory08": ("Item8Category", True, ITEMLOT_ITEMCATEGORY, "Type of item (slot 8)."),
        "lotItemId08": partial(_item_lot_item_id, 8),
        "lotItemNum08": ("Item8Count", True, int, "Count of item (slot 8)."),
        "lotItemBasePoint08": (
            "Item8ChancePoints",
            True,
            int,
    ...
    "NPC_PARAM_ST": {
        ...
        "aiThinkId": ("AiThinkID", True, Params.AI, "Default AI ID. Overridden by AI ID field in Maps entry."),
        "nameId": (
            "NameID",
            True,
            Text.NPCNames,
        ...
        "hitHeight": ("HitHeight", True, float, "Height of NPC hitbox for collision."),
        "hitRadius": ("HitRadius", True, float, "Radius of NPC hitbox for collision."),
        "weight": ("Weight", False, int, "Weight of NPC. Probably has no effect (generall 100)."),
        "hitYOffset": ("HitYOffset", False, float, "Vertical offset of NPC hitbox for collision."),
        "hp": ("MaximumHP", True, int, "Maximum HP of NPC."),
        "mp": ("MaximumMP", False, int, "Maximum MP of NPC. Not used in Dark Souls (generally zero)."),
        ...
        "moveAnimId": ("MoveAnimationID", False, Animation, "DOC-TODO"),
        "spMoveAnimId1": ("SpecialMoveAnimationID1", False, Animation, "DOC-TODO"),
        "spMoveAnimId2": ("SpecialMoveAnimationID2", False, Animation, "DOC-TODO"),
        "networkWarpDist": ("NetworkWarpDistance", False, float, "DOC-TODO"),
        "dbgBehaviorR1": ("DebugBehaviorR1", False, Animation, "DOC-TODO"),
        "dbgBehaviorL1": ("DebugBehaviorL1", False, Animation, "DOC-TODO"),
        "dbgBehaviorR2": ("DebugBehaviorR2", False, Animation, "DOC-TODO"),
        "dbgBehaviorL2": ("DebugBehaviorL2", False, Animation, "DOC-TODO"),
        "dbgBehaviorRL": ("DebugBehaviorRL", False, Animation, "DOC-TODO"),
        "dbgBehaviorRR": ("DebugBehaviorRR", False, Animation, "DOC-TODO"),
        "dbgBehaviorRD": ("DebugBehaviorRD", False, Animation, "DOC-TODO"),
        "dbgBehaviorRU": ("DebugBehaviorRU", False, Animation, "DOC-TODO"),
        "dbgBehaviorLL": ("DebugBehaviorLL", False, Animation, "DOC-TODO"),
        "dbgBehaviorLR": ("DebugBehaviorLR", False, Animation, "DOC-TODO"),
        "dbgBehaviorLD": ("DebugBehaviorLD", False, Animation, "DOC-TODO"),
        "dbgBehaviorLU": ("DebugBehaviorLU", False, Animation, "DOC-TODO"),
        "animIdOffset2": ("AnimationIDOffset2", False, int, "DOC-TODO"),
        "partsDamageRate1": (
        ...
        "stamina": ("MaxStamina", True, int, "Maximum stamina of NPC."),
        "staminaRecoverBaseVel": ("StaminaRecoveryBaseSpeed", True, int, "Base speed of NPC's stamina recovery."),
        "def_phys": ("PhysicalDefense", True, int, "Base defense applied to all physical attacks."),
        "def_slash": ("SlashDefense", True, int, "Base defense added against slashing physical attacks."),
        "def_blow": ("StrikeDefense", True, int, "Base defense added against striking physical attacks."),
        "def_thrust": ("ThrustDefense", True, int, "Base defense added against thrusting physical attacks."),
        "def_mag": ("MagicDefense", True, int, "Base defense added against magic attacks."),
        "def_fire": ("FireDefense", True, int, "Base defense added against fire attacks."),
        "def_thunder": ("LightningDefense", True, int, "Base defense added against lightning attacks."),
        "defFlickPower": (
            "DefenseRepelPower",
            False,
            int,
        ...
        "resist_poison": ("PoisonResistance", True, int, "Base poison resistance."),
        "resist_desease": ("ToxicResistance", True, int, "Base toxic resistance."),
        "resist_blood": ("BleedResistance", True, int, "Base bleed resistance."),
        "resist_curse": ("CurseResistance", True, int, "Base curse resistance."),
        "ghostModelId": (
            "GhostModelID",
            False,
            Model,
        ...
        "normalChangeResouceId": ("NormalChangeResourceID", False, int, "Unknown. Always -1."),
        "guardAngle": ("GuardAngle", False, int, "Zero for every NPC except the Phalanx Hollow (20)."),
        "slashGuardCutRate": ("SlashDamageReductionWhenGuarding", False, int, "Always zero."),
        "blowGuardCutRate": ("StrikeDamageReductionWhenGuarding", False, int, "Always zero."),
        "thrustGuardCutRate": ("ThrustDamageReductionWhenGuarding", False, int, "Always zero."),
        ...
        "knockbackRate": ("KnockbackRate", True, int, "DOC-TODO"),
        "knockbackParamId": (
            "KnockbackID",
            True,
            Params.Knockback,
        ...
        "fallDamageDump": ("FallDamageReduction", True, int, "Percentage of fall damage to ignore."),
        "staminaGuardDef": (
            "StaminaGuardDefense",
            True,
            int,
        ...
        "pcAttrB": ("PCAttrB", False, int, "Like a remnant of World Tendency. Always set to zero."),
        "pcAttrW": ("PCAttrW", False, int, "Like a remnant of World Tendency. Always set to zero."),
        "pcAttrL": ("PCAttrL", False, int, "Like a remnant of World Tendency. Always set to zero."),
        "pcAttrR": ("PCAttrR", False, int, "Like a remnant of World Tendency. Always set to zero."),
        "areaAttrB": ("AreaAttrB", False, int, "Like a remnant of World Tendency. Always set to zero."),
        "areaAttrW": ("AreaAttrW", False, int, "Like a remnant of World Tendency. Always set to zero."),
        "areaAttrL": ("AreaAttrL", False, int, "Like a remnant of World Tendency. Always set to zero."),
        "areaAttrR": ("AreaAttrR", False, int, "Like a remnant of World Tendency. Always set to zero."),
        "mpRecoverBaseVel": (
            "MPRecoveryBaseSpeed",
            False,
            int,
        ...
        "drawType": ("DrawType", True, NPC_DRAW_TYPE, "DOC-TODO"),
        "npcType": ("NPCType", True, NPC_TYPE, "DOC-TODO"),
        "teamType": ("TeamType", True, NPC_TEMA_TYPE, "0: enemy, 1: boss, 2: ally, 6: unused, 7: white phantom"),
        "moveType": ("MoveType", True, NPC_MOVE_TYPE, "DOC-TODO"),
        "lockDist": ("LockOnDistance", True, int, "DOC-TODO"),
        "material": ("Material", False, int, "DOC-TODO"),
        "materialSfx": ("MaterialSFX", False, int, "DOC-TODO"),
        "material_Weak": ("MaterialWeak", False, int, "DOC-TODO"),
        "materialSfx_Weak": ("MaterialWeakSFX", False, int, "DOC-TODO"),
        "partsDamageType": (
            "PartsDamageType",
            False,
            int,
        ...
        "guardLevel": (
            "GuardLevel",
            False,
            GUARDMOTION_CATEGORY,  # TODO: possibly GUARDMOTION_CATEGORY
        ...
        "bloodGuardResist": ("BleedGuardResistance", True, int, "Added bleed resistance while guarding."),
        "curseGuardResist": ("CurseGuardResistance", True, int, "Added curse resistance while guarding."),
        "parryAttack": ("ParryAttack", False, int, "Always zero."),
        "parryDefence": ("ParryDefense", False, int, "Always zero."),
        "sfxSize": (
            "SFXSize",
            False,
            NPC_SFX_SIZE,
        ...
        "pushOutCamRegionRadius": ("PushOutCameraRegionRadius", False, int, "Always zero."),
        "hitStopType": (
            "HitStopType",
            False,
            NPC_HITSTOP_TYPE,
        ...
        "ladderEndChkOffsetLow": ("LadderEndCheckOffsetBottom", False, int, "Not something you want to mess with."),
        "useRagdollCamHit:1": ("UseRagdollCameraHit", False, bool, "DOC-TODO"),
        "disableClothRigidHit:1": ("DisableClothRigidHit", False, bool, "DOC-TODO"),
        "useRagdoll:1": ("UseRagdoll", False, bool, "DOC-TODO"),
        "isDemon:1": ("IsDemon", True, bool, "DOC-TODO"),
        "isGhost:1": ("IsGhost", True, bool, "DOC-TODO"),
        "isNoDamageMotion:1": ("IsNoDamageMotion", True, bool, "DOC-TODO"),
        "isUnduration:1": ("IsUnduration", False, bool, "DOC-TODO"),
        "isChangeWanderGhost:1": ("IsChangeWanderGhost", False, bool, "Always false."),
        "modelDispMask0:1": ("ModelDisplayMask0", False, bool, "DOC-TODO"),
        "modelDispMask1:1": ("ModelDisplayMask1", False, bool, "DOC-TODO"),
        "modelDispMask2:1": ("ModelDisplayMask2", False, bool, "DOC-TODO"),
        "modelDispMask3:1": ("ModelDisplayMask3", False, bool, "DOC-TODO"),
        "modelDispMask4:1": ("ModelDisplayMask4", False, bool, "DOC-TODO"),
        "modelDispMask5:1": ("ModelDisplayMask5", False, bool, "DOC-TODO"),
        "modelDispMask6:1": ("ModelDisplayMask6", False, bool, "DOC-TODO"),
        "modelDispMask7:1": ("ModelDisplayMask7", False, bool, "DOC-TODO"),
        "modelDispMask8:1": ("ModelDisplayMask8", False, bool, "DOC-TODO"),
        "modelDispMask9:1": ("ModelDisplayMask9", False, bool, "DOC-TODO"),
        "modelDispMask10:1": ("ModelDisplayMask10", False, bool, "DOC-TODO"),
        "modelDispMask11:1": ("ModelDisplayMask11", False, bool, "DOC-TODO"),
        "modelDispMask12:1": ("ModelDisplayMask12", False, bool, "DOC-TODO"),
        "modelDispMask13:1": ("ModelDisplayMask13", False, bool, "DOC-TODO"),
        "modelDispMask14:1": ("ModelDisplayMask14", False, bool, "DOC-TODO"),
        "modelDispMask15:1": ("ModelDisplayMask15", False, bool, "DOC-TODO"),
        "isEnableNeckTurn:1": ("IsEnableNeckTurn", False, bool, "DOC-TODO"),
        "disableRespawn:1": (
        ...
        "isMoveAnimWait:1": ("IsMoveAnimationWait", False, bool, "DOC-TODO"),
        "isCrowd:1": ("IsCrowd", False, bool, "Always false."),
        "isWeakSaint:1": (
            "IsWeakToDivine",
            True,
            bool,
        ...
        "isWeakA:1": ("IsWeakToOccult", True, bool, "True for all Gods and most NPCs in Anor Londo."),
        "isWeakB:1": (
            "IsAbyssal",
            True,
            bool,
        ...
        "pad1:1": ("Pad1", False, "<Pad:1>", "DOC-TODO"),
        "vowType:3": (
            "VowType",
            False,
            int,
        ...
        "pad3:4": ("Pad2", False, "<Pad:4>", "DOC-TODO"),
        "pad2[6]": ("Pad3", False, "<Pad:6>", "DOC-TODO"),
    },
    "EQUIP_PARAM_ACCESSORY_ST": {
        "refId": ("SpecialEffect", True, Params.SpecialEffects, "Special effect applied when ring is equipped."),
        "sfxVariationId": (
            "SFXVariation",
            False,
            int,
        ...
        "basicPrice": ("BasicCost", False, int, "Unknown purpose, and unused."),
        "sellValue": (
            "FramptSellValue",
            True,
            int,
        ...
        "sortId": ("SortIndex", True, int, "Index for automatic inventory sorting."),
        "qwcId": ("QWCID", False, int, "Unused world tendency remnant."),
        "equipModelId": ("EquipmentModel", False, Model, "Always zero. (Rings have no model, presumably.)"),
        "iconId": ("MenuIcon", True, Texture, "Icon ID of ring in menu."),
        "shopLv": (
            "ShopLevel",
            False,
            int,
        ...
        "equipModelCategory": ("EquipmentModelCategory", False, EQUIP_MODEL_CATEGORY, "Always zero."),
        "equipModelGender": ("EquipmentModelGender", False, EQUIP_MODEL_GENDER, "Always zero."),
        "accessoryCategory": ("AccessoryCategory", False, ACCESSORY_CATEGORY, "Always zero."),
        "refCategory": (
            "ReferenceType",
            False,
            BEHAVIOR_REF_TYPE,
        ...
        "pad1[3]": ("Pad2", False, "<Pad:3>", "Null padding."),
    },
    "OBJECT_PARAM_ST": {
        "hp": (
            "ObjectHP",
        ...
        "isLadder:1": ("IsLadder", True, bool, "Object is a ladder."),
        ...
        "isMoveObj:1": ("IsMovingObject", True, bool, "If True, this object can move."),
        "pad_1:5": ("Pad1", False, "<Pad:5>", "Null padding."),
        "defaultLodParamId": ("DefaultLOD", True, int, "Default LOD (level of default) parameter."),  # TODO: LOD.
        "breakSfxId": (
    ...
    "OBJ_ACT_PARAM_ST": {
        ...
        "pad1[2]": ("Pad1", False, "<Pad:2>", "Null padding."),
    },
    "SHOP_LINEUP_PARAM": {
        "equipId": _shop_item_id,
        "value": ("SoulCost", True, int, "Cost of item, in souls."),
        "mtrlId": (
            "RequiredGood",
            True,
            Params.Goods,
        ...
        "qwcId": ("QWCID", False, int, "Unused world tendency condition."),
        "sellQuantity": (
            "InitialQuantity",
            True,
            int,
        ...
        "equipType": ("ItemType", True, SHOP_LINEUP_EQUIPTYPE, "Type of item listed in menu."),
        "pad_0[8]": ("Pad1", False, "<Pad:8>", "Null padding."),
    },
    "SP_EFFECT_PARAM_ST": {
        "iconId": (
            "StatusIcon",
        ...
        "maxHpRate": ("MaxHPMultiplier", True, float, "Multiplier applied to maximum HP."),
        "maxMpRate": (
            "MaxMPMultiplier",
            False,
            float,
        ...
        "maxStaminaRate": ("MaxStaminaMultiplier", True, float, "Multiplier applied to maximum stamina."),
        "slashDamageCutRate": (
        ...
        "fireAttackRate": ("OutgoingFireDamageMultiplier", True, float, "Multiplier applied to outgoing fire damage."),
        "thunderAttackRate": (
        ...
        "fireDiffenceRate": ("FireDefenseMultiplier", True, float, "Multiplier applied to character's fire defense."),
        "thunderDiffenceRate": (
            "LightningDefenseMultiplier",
            True,
            float,
        ...
        "insideDurability": (
            ...
            "of the special effect. The equipment affected is determined by... ",
        ),  # TODO
        "maxDurability": (
        ...
        "guardDefFlickPowerRate": ("GuardDefenseFlickPowerRate", False, float, "Unknown; never used."),
        "guardStaminaCutRate": (
        ...
        "stateInfo": (
            "SpecialState",
            True,
            SpecialStateInfo,  # TODO: right click can edit numeric entry directly.
        ...
        "effectTargetSelf:1": ("CanAffectSelf", True, bool, "Effect will target self."),
        "effectTargetFriend:1": ("CanAffectAlly", True, bool, "Effect will target self."),
        "effectTargetEnemy:1": ("CanAffectEnemy", True, bool, "Effect will target enemies."),
        "effectTargetPlayer:1": ("CanAffectPlayer", True, bool, "Effect will target player characters."),
        "effectTargetAI:1": ("CanAffectAI", True, bool, "Effect will target non-player characters."),
        "effectTargetLive:1": ("CanAffectPlayers", True, bool, "Effect will target humans."),
        ...
        "clearSoul:1": ("ClearSoul", False, bool, "Unused Demon's Souls remnant."),
        "requestSOS:1": ("RequestWhitePhantomSummon", False, bool, "Used only by White Sign Soapstone."),
        ...
        "requestLeaveSession:1": ("RequestReturnToOwnWorld", False, bool, "Used only by Black Separation Crystal."),
        "requestNpcInveda:1": (
            "RequestNPCInvasion",
            False,
            bool,
        ...
        "noDead:1": ("Immortal", True, bool, "If True, character cannot die. Never used in vanilla game."),
        ...
        "fakeTargetIgnore:1": ("IgnoreFakeTargets", False, bool, "Unknown; never used."),
        "fakeTargetIgnoreUndead:1": ("IgnoreUndeadFakeTargets", False, bool, "Unknown; never used."),
        "fakeTargetIgnoreAnimal:1": ("IgnoreBeastFakeTargets", False, bool, "Unknown; never used."),
        "grabityIgnore:1": ("IgnoreGravity", True, bool, "Ignore gravity. (Not sure if this actually works.)"),
        "disablePoison:1": ("PoisonImmunity", True, bool, "Immune to poison."),
        "disableDisease:1": ("ToxicImmunity", True, bool, "Immune to toxic."),
        "disableBlood:1": ("BleedImmunity", True, bool, "Immune to curse."),
        "disableCurse:1": ("CurseImmunity", True, bool, "Immune to poison."),
        "enableCharm:1": (
            "EnableCharming",
            False,
            bool,
        ...
        "requestLeaveColiseumSession:1": ("RequestColiseumExit", False, bool, "Used only by Purple Coward's Crystal."),
        "pad_2:4": ("Pad1", False, "<Pad:4>", "Null padding."),
        "vowType0:1": (
        ...
        "pad1[11]": ("Pad2", False, "<Pad:11>", "Null padding."),
    },
    "MAGIC_PARAM_ST": {
        "yesNoDialogMessageId": (
            "ConfirmationMessage",
        ...
        "limitCancelSpEffectId": ("LimitCancelSpecialEffect", False, Params.SpecialEffects, "Unknown. Never used."),
        "sortId": ("SortIndex", True, int, "Index for automatic inventory sorting."),
        "refId": _spells_ref_id,
        "mp": ("MPCost", False, int, "MP cost of spell. Unused in Dark Souls 1 (always zero)."),
        "stamina": ("StaminaCost", False, int, "Stamina cost of spell. Always zero."),
        "iconId": ("SpellIcon", True, Texture, "Spell icon texture for inventory and equipped slot."),
        "behaviorId": ("Behavior", False, Params.Behaviors, "Behavior triggered by spell. Never used."),
        "mtrlItemId": (
            "RequiredGood",
            False,
            Params.Goods,
        ...
        "overDexterity": ("OverDexterity", False, int, "Unknown effect. Always 99."),
        "sfxVariationId": (
        ...
        "requirementIntellect": ("RequiredIntelligence", True, int, "Minimum intelligence required to cast spell."),
        "requirementFaith": ("RequiredFaith", True, int, "Minimum faith required to cast spell."),
        "analogDexiterityMin": (
        ...
        "ezStateBehaviorType": ("SpellCategory", True, MAGIC_CATEGORY, "Type of spell."),
        "refCategory": (
            "ReferenceType",
            True,
            BEHAVIOR_REF_TYPE,
        ...
        "isEnchant:1": ("IsWeaponBuff", True, bool, "Indicates if this spell buffs your weapon."),
        "isShieldEnchant:1": ("IsShieldBuff", True, bool, "Indicates if this spell buffs your shield."),
        "enable_live:1": (
            "UseableByHumans",
            True,
            bool,
        ...
        "pad_1:6": ("Pad1", False, "<Pad:6>", "Null padding."),
        "pad[2]": ("Pad2", False, "<Pad:2>", "Null padding."),
    },
    "THROW_INFO_BANK": {
        "AtkChrId": ("AttackingCharacterModel", True, Model, "Model ID of attacking character."),
        "DefChrId": ("DefendingCharacterModel", True, Model, "Model ID of defending character."),
        "Dist": ("MaxDistance", True, float, "Maximum distance at which throw can be triggered."),
        "DiffAngMin": (
        ...
        "atkAnimId": ("AttackerAnimation", True, Animation, "Animation played by attacker during throw."),
        "defAnimId": ("DefenderAnimation", True, Animation, "Animation played by defender during throw."),
        "escHp": (
        ...
        "pad0:5": ("Pad1", False, "<Pad:5>", "Null padding."),
        "pad1[4]": ("Pad2", False, "<Pad:4>", "Null padding."),
    },
    "EQUIP_MTRL_SET_PARAM_ST": {
        "materialId01": ("UpgradeGood", True, Params.Goods, "Good required to upgrade weapon."),
        "materialId02": (
            "UpgradeGood2",
            False,
            Params.Goods,
        ...
        "itemNum01": ("UpgradeQuantity", True, int, "Amount of Upgrade Good required for reinforcement."),
        "itemNum02": (
            "UpgradeQuantity2",
            False,
            int,
    ...
    "EQUIP_PARAM_WEAPON_ST": {
        ...
        "sortId": ("SortIndex", True, int, "Index for automatic inventory sorting."),
        "wanderingEquipId": ("GhostWeaponReplacement", True, Params.Weapons, "Weapon replacement for ghosts."),
        "weight": ("Weight", True, float, "Weight of weapon."),
        "weaponWeightRate": (
            "WeightRatio",
            True,
            float,
        ...
        "vagrantBonusEneDropItemLotId": ("VagrantBonusEnemyDropItemLot", False, Params.ItemLots, "DOC-TODO"),
        "vagrantItemEneDropItemLotId": ("VagrantItemEnemyDropItemLot", False, Params.ItemLots, "DOC-TODO"),
        "equipModelId": ("WeaponModel", True, Model, "Weapon model ID."),
        "iconId": ("WeaponIcon", True, Texture, "Weapon icon texture ID."),
        "durability": (
            "InitialDurability",
            True,
            int,
        ...
        "durabilityMax": ("MaxDurability", True, int, "Maximum durability of weapon."),
        "attackThrowEscape": (
            "ThrowEscapePower",
            False,
            int,
        ...
        "attackBasePhysics": ("BasePhysicalDamage", True, int, "Base physical damage of weapon attacks."),
        "attackBaseMagic": ("BaseMagicDamage", True, int, "Base magic damage of weapon attacks."),
        "attackBaseFire": ("BaseFireDamage", True, int, "Base fire damage of weapon attacks."),
        "attackBaseThunder": ("BaseLightningDamage", True, int, "Base lightning damage of weapon attacks."),
        "attackBaseStamina": ("BaseStaminaDamage", True, int, "Base stamina damage of weapon attacks."),
        "saWeaponDamage": ("BasePoiseDamage", True, int, "Base poise damage of weapon attacks."),
        "saDurability": (
            "AttackPoiseBonus",
            False,
            int,
        ...
        "guardAngle": ("EffectiveGuardAngle", False, int, "Angle that can be guarded with this weapon. Never used."),
        "staminaGuardDef": (
            "GuardStaminaDefense",
            True,
            int,
        ...
        "spAttribute": ("ElementAttribute", True, ATKPARAM_SPATTR_TYPE, "Element attached to hits with this weapon."),
        "spAtkcategory": (
        ...
        "properAgility": ("RequiredDexterity", True, int, "Required dexterity to wield weapon properly."),
        "properMagic": ("RequiredIntelligence", True, int, "Required intelligence to wield weapon properly."),
        "properFaith": ("RequiredFaith", True, int, "Required faith to wield weapon properly."),
        "overStrength": ("OverStrength", False, int, "Unknown. Always set to 99, except for arrows and bolts."),
        "attackBaseParry": ("AttackBaseParry", False, int, "Unknown. Never used."),
        "defenseBaseParry": ("DefenseBaseParry", False, int, "Unknown. Never used."),
        "guardBaseRepel": (
            "DeflectOnBlock",
            True,
            int,
        ...
        "bothHandEquipable:1": ("BothHandsAllowed", True, bool, "If True, this weapon can be held in two-handed mode."),
        ...
        "boltSlotEquipable:1": ("UsesEquippedBolts", True, bool, "If True, this weapon will use equipped bolt slot."),
        "enableGuard:1": ("GuardEnabled", True, bool, "If True, the player can guard with this weapon by holding L1."),
        "enableParry:1": ("ParryEnabled", True, bool, "If True, the player can parry with this weapon by pressing L2."),
        ...
        "enableVowMagic:1": ("CanCastCovenantMagic", True, bool, ""),
        "isNormalAttackType:1": ("DealsNeutralDamage", True, bool, ""),
        "isBlowAttackType:1": ("DealsStrikeDamage", True, bool, ""),
        "isSlashAttackType:1": ("DealsSlashDamage", True, bool, ""),
        "isThrustAttackType:1": ("DealsThrustDamage", True, bool, ""),
        "isEnhance:1": ("IsUpgraded", True, bool, ""),
        "isLuckCorrect:1": ("IsAffectedByLuck", True, bool, ""),
        "isCustom:1": ("IsCustom?", True, bool, ""),
        "disableBaseChangeReset:1": ("DisableBaseChangeReset", True, bool, ""),
        "disableRepair:1": ("DisableRepairs", True, bool, "If True, this weapon cannot be repaired."),
        "isDarkHand:1": ("IsDarkHand", False, bool, "Enabled only for the Dark Hand."),
        "simpleModelForDlc:1": ("SimpleDLCModelExists", False, bool, "Unknown; always set to False."),
        "lanternWep:1": ("IsLantern?", True, bool, ""),
        "isVersusGhostWep:1": (
            "CanHitGhosts",
            True,
            bool,
        ...
        "isDragonSlayer:1": ("IsDragonSlayer", True, bool, ""),
        "isDeposit:1": (
            "CanBeStored",
            True,
            bool,
        ...
        "pad_1[8]": ("Pad1", False, "<Pad:8>", "Null padding."),
        "disableMultiDropShare:1": (
            "DisableMultiplayerShare",
            False,
            bool,
        ...
        "pad_0[1]": ("Pad2", False, "<Pad:1>", "Null padding."),
        "oldSortId": ("OldSortIndex", False, int, "Sorting index for an obsolete build of the game. No effect."),
        "levelSyncCorrectID": (
            "LevelSyncCorrection",
            False,
        ...
        "pad_1[6]": ("Pad3", False, "<Pad:6>", "Null padding."),
    },
    ...
    "MOVE_PARAM_ST": {
        "stayId": ("StillAnimation", True, Animation, "Animation ID."),
        "walkF": ("WalkForwardAnimatiom", True, Animation, "Animation ID."),
        "walkB": ("WalkBackwardAnimation", True, Animation, "Animation ID."),
        "walkL": ("StrafeLeftAnimation", True, Animation, "Animation ID."),
        "walkR": ("StrafeRightAnimation", True, Animation, "Animation ID."),
        "dashF": ("RunForwardAnimation", True, Animation, "Animation ID."),
        "dashB": ("RunBackwardAnimation", True, Animation, "Animation ID."),
        "dashL": ("StrafeRunLeftAnimation", True, Animation, "Animation ID."),
        "dashR": ("StrafeRunRightAnimation", True, Animation, "Animation ID."),
        "superDash": ("SprintForwardAnimation", True, Animation, "Animation ID."),
        "escapeF": ("RollForwardAnimation", True, Animation, "Animation ID."),
        "escapeB": ("RollBackwardAnimation", True, Animation, "Animation ID."),
        "escapeL": ("RollLeftAnimation", True, Animation, "Animation ID."),
        "escapeR": ("RollRightAnimation", True, Animation, "Animation ID."),
        "turnL": ("TurnLeftAnimation", True, Animation, "Animation ID."),
        "trunR": ("TurnRightAnimation", True, Animation, "Animation ID."),
        "largeTurnL": ("LargeTurnLeftAnimation", True, Animation, "Animation ID."),
        "largeTurnR": ("LargeTurnRightAnimation", True, Animation, "Animation ID."),
        "stepMove": ("BackstepAnimation", True, Animation, "Animation ID."),
        "flyStay": ("HoverAnimation", True, Animation, "Animation ID."),
        "flyWalkF": ("FlyForwardAnimation", True, Animation, "Animation ID."),
        "flyWalkFL": ("FlyForwardLeftAnimation", True, Animation, "Animation ID."),
        "flyWalkFR": ("FlyForwardRightAnimation", True, Animation, "Animation ID."),
        "flyWalkFL2": ("FlyForwardLeft2Animation", True, Animation, "Animation ID."),
        "flyWalkFR2": ("FlyForwardRight2Animation", True, Animation, "Animation ID."),
        "flyDashF": ("FlyForwardFastAnimation", True, Animation, "Animation ID."),
        "flyDashFL": ("FlyForwardLeftFastAnimation", True, Animation, "Animation ID."),
        "flyDashFR": ("FlyForwardRightFastAnimation", True, Animation, "Animation ID."),
        "flyDashFL2": ("FlyForwardLeftFast2Animation", True, Animation, "Animation ID."),
        "flyDashFR2": ("FlyForwardRightFast2Animation", True, Animation, "Animation ID."),
        "dashEscapeF": ("RunningRollForwardAnimation", True, Animation, "Animation ID."),
        "dashEscapeB": ("RunningRollBackwardAnimation", True, Animation, "Animation ID. (Never used.)"),
        "dashEscapeL": ("RunningRollLeftAnimation", True, Animation, "Animation ID. (Never used.)"),
        "dashEscapeR": ("RunningRollRightAnimation", True, Animation, "Animation ID. (Never used.)"),
        "analogMoveParamId": (
            "AnalogMovement",
            True,
            int,
        ...
        "pad[4]": ("Pad1", False, "<Pad:4>", "Null padding."),
    },
    "SP_EFFECT_VFX_PARAM_ST": {
        ...
        "existEffectForSoul:1": ("SoulEffectExists", True, bool, "Indicates if a 'soul version' of the effect exists."),
        ...
        "invisibleAtFriendCamouflage:1": ("InvisibleWhenFriendHidden", True, bool, "Unclear."),
        ...
        "isInvisibleWeapon:1": ("HideWeapon", True, bool, "Weapon is invisible if enabled."),
        "isSilence:1": ("IsSilent", True, bool, "Movement noises are silenced if enabled."),
        "pad_1:6": ("Pad1", False, "<Pad:6>", "Null padding."),
        "pad[16]": ("Pad2", False, "<Pad:16>", "Null padding."),
    },
    "CACL_CORRECT_GRAPH_ST": {
        ...
        "stageMaxGrowVal0": ("StageMaxSlope0", True, float, "Slope in equation of 'stage max' line 0."),
        "stageMaxGrowVal1": ("StageMaxSlope1", True, float, "Slope in equation of 'stage max' line 1."),
        "stageMaxGrowVal2": ("StageMaxSlope2", True, float, "Slope in equation of 'stage max' line 2."),
        "stageMaxGrowVal3": ("StageMaxSlope3", True, float, "Slope in equation of 'stage max' line 3."),
        "stageMaxGrowVal4": ("StageMaxSlope4", True, float, "Slope in equation of 'stage max' line 4."),
        "adjPt_maxGrowVal0": (
            "AdustmentMaxSlope0",
            True,
            float,
    ...
    "FOG_BANK": {
        "fogBeginZ": ("FogStartDistance", True, int, "Distance (m) at which fog begins."),
        "fogEndZ": ("FogEndDistance", True, int, "Distance (m) at which fog ends."),
        "degRotZ": ("RotationZ", True, int, "Rotation of fog around the Z axis."),
        "degRotW": ("RotationW", True, int, "Rotation of fog around the W axis."),
        "colR": ("Red", True, int, "Red color channel (0-255)."),
        "colG": ("Green", True, int, "Green color channel (0-255)."),
        "colB": ("Blue", True, int, "Blue color channel (0-255)."),
        "colA": ("Alpha", True, int, "Alpha channel (0-255)."),
    },
    "LIGHT_BANK": {
        "degRotX_0": ("AmbientLight0RotationX", False, int, "Rotation (X-axis) of ambient (parallle) light source 0."),
        "degRotY_0": ("AmbientLight0RotationY", False, int, "Rotation (Y-axis) of ambient (parallle) light source 0."),
        "colR_0": ("AmbientLight0Red", False, int, "Red channel (0-255) of ambient (parallle) light source 0."),
        "colG_0": ("AmbientLight0Green", False, int, "Green channel (0-255) of ambient (parallle) light source 0."),
        "colB_0": ("AmbientLight0Blue", False, int, "Blue channel (0-255) of ambient (parallle) light source 0."),
        "colA_0": ("AmbientLight0Alpha", False, int, "Alpha channel (0-255) of ambient (parallle) light source 0."),
        "degRotX_1": ("AmbientLight1RotationX", False, int, "Rotation (X-axis) of ambient (parallle) light source 1."),
        "degRotY_1": ("AmbientLight1RotationY", False, int, "Rotation (Y-axis) of ambient (parallle) light source 1."),
        "colR_1": ("AmbientLight1Red", False, int, "Red channel (0-255) of ambient (parallle) light source 1."),
        "colG_1": ("AmbientLight1Green", False, int, "Green channel (0-255) of ambient (parallle) light source 1."),
        "colB_1": ("AmbientLight1Blue", False, int, "Blue channel (0-255) of ambient (parallle) light source 1."),
        "colA_1": ("AmbientLight1Alpha", False, int, "Alpha channel (0-255) of ambient (parallle) light source 1."),
        "degRotX_2": ("AmbientLight2RotationX", False, int, "Rotation (X-axis) of ambient (parallle) light source 2."),
        "degRotY_2": ("AmbientLight2RotationY", False, int, "Rotation (Y-axis) of ambient (parallle) light source 2."),
        "colR_2": ("AmbientLight2Red", False, int, "Red channel (0-255) of ambient (parallle) light source 2."),
        "colG_2": ("AmbientLight2Green", False, int, "Green channel (0-255) of ambient (parallle) light source 2."),
        "colB_2": ("AmbientLight2Blue", False, int, "Blue channel (0-255) of ambient (parallle) light source 2."),
        "colA_2": ("AmbientLight2Alpha", False, int, "Alpha channel (0-255) of ambient (parallle) light source 2."),
        "colR_u": (
            "TopDownLightRed",
            True,
            int,
        ...
        "colG_s": ("SpecularAmbientLightGreen", True, int, "Green channel (0-255) of specular ambient light source."),
        "colB_s": ("SpecularAmbientLightBlue", True, int, "Blue channel (0-255) of specular ambient light source."),
        "colA_s": ("SpecularAmbientLightAlpha", True, int, "Alpha channel (0-255) of specular ambient light source."),
        "envDif_colR": ("DiffuseLightRed", True, int, "Red channel (0-255) of base diffuse ambient light of surfaces."),
        "envDif_colG": (
            "DiffuseLightGreen",
            True,
            int,
        ...
        "envSpc_colR": ("SpecularLightRed", True, int, "Red channel (0-255) of specular ambient light of surfaces."),
        "envSpc_colG": (
            "SpecularLightGreen",
            True,
            int,
        ...
        "envSpc_colB": ("SpecularLightBlue", True, int, "Blue channel (0-255) of specular ambient light of surfaces."),
        "envSpc_colA": (
            "SpecularLightAlpha",
            True,
            int,
        ...
        "pad[2]": ("Pad1", False, "<Pad:2>", "Null padding."),
    },
    "LIGHT_SCATTERING_BANK": {
        "sunRotX": ("LightRotationX", True, int, "Rotation (X-axis) of scattering light source."),
        "sunRotY": ("LightRotationY", True, int, "Rotation (Y-axis) of scattering light source."),
        "distanceMul": ("DistanceMultiplier", True, int, "Magnification (0-100) of scattering light source distance."),
        "sunR": ("LightRed", True, int, "Red channel (0-255) of scattering light source."),
        "sunG": ("LightGreen", True, int, "Green channel (0-255) of scattering light source."),
        "sunB": ("LightBlue", True, int, "Blue channel (0-255) of scattering light source."),
        "sunA": ("LightAlpha", True, int, "Alpha channel (0-255) of scattering light source."),
        "pad_0[2]": ("Pad0", False, "<Pad:2>", "Null padding."),
        "lsHGg": (
            "ScatteringDirection",
            True,
            float,
        ...
        "pad_1[2]": ("Pad1", False, "<Pad:2>", "Null padding."),
    },
    "POINT_LIGHT_BANK": {
        "dwindleBegin": ("FadeStartDistance", True, float, "Distance at which player's point light begins to fade."),
        "dwindleEnd": (
            "FadeEndDistance",
            True,
            float,
        ...
        "colR": ("PointLightRed", True, int, "Red channel (0-255) of point light."),
        "colG": ("PointLightGreen", True, int, "Green channel (0-255) of point light."),
        "colB": ("PointLightBlue", True, int, "Blue channel (0-255) of point light."),
        "colA": ("PointLightAlpha", True, int, "Alpha channel (0-255) of point light."),
    },
    "LENS_FLARE_BANK": {
        "texId": (
            "LensFlareTextureID",
        ...
        "isFlare": ("IsLensFlare", True, bool, "Flare if enabled, or 'ghost' if disabled."),
        "enableRoll": ("EnableRotation", True, bool, "Allows lens flare texture to rotate with camera."),
        ...
        "texScale": ("TextureScale", True, float, "Base scaling of lens flare texture."),
        "colR": ("TextureRed", True, int, "Red channel (0-255) of lens flare texture."),
        "colG": ("TextureGreen", True, int, "Green channel (0-255) of lens flare texture."),
        "colB": ("TextureBlue", True, int, "Blue channel (0-255) of lens flare texture."),
        "colA": ("TextureAlpha", True, int, "Alpha channel (0-255) of lens flare texture."),
    },
    "LENS_FLARE_EX_BANK": {
        "lightDegRotX": (
    ...
    "DOF_BANK": {
        "farDofBegin": ("FarBlurStartDistance", True, float, "Distance (m) at which far depth of field blur begins."),
        "farDofEnd": (
            "FarBlurEndDistance",
            True,
            float,
        ...
        "pad_0[3]": ("Pad0", False, "<Pad:3>", "Null padding."),
        "nearDofBegin": (
        ...
        "pad_1[3]": ("Pad1", False, "<Pad:3>", "Null padding."),
        "dispersionSq": (
    ...
    "TONE_MAP_BANK": {
        ...
        "bloomMul": ("NearBloomMultiplier", True, int, "Near light blooming multiplier."),
        "bloomBeginFar": (
            "FarBloomThreshold",
            True,
            int,
        ...
        "bloomMulFar": ("FarBloomMultiplier", True, int, "Far light blooming multiplier."),
        "bloomNearDist": (
        ...
        "adaptSpeed": ("AdaptationSpeed", True, float, "Tone adaptation speed."),
        "lightShaftBegin": (
            "LightShaftThreshold",
            True,
            int,
        ...
        "pad_0[3]": ("Pad0", True, "<Pad:3>", "Null padding."),
        "lightShaftPower": ("LightShaftMagnitude", True, float, "Light shaft magnitude (0 means no light shafts)."),
        "lightShaftAttenRate": (
    ...
    "TONE_CORRECT_BANK": {
        ...
        "contrastR": ("ContrastRed", True, float, "Red channel (0-255) of tone correction contrast."),
        "contrastG": ("ContrastGreen", True, float, "Green channel (0-255) of tone correction contrast."),
        "contrastB": ("ContrastBlue", True, float, "Blue channel (0-255) of tone correction contrast."),
        "saturation": ("SaturationCorrection", True, float, "Color saturation correction value."),
        "hue": ("HueCorrection", True, float, "Color hue correction value."),
    },
    "SHADOW_BANK": {
        ...
        "colR": ("ShadowRed", True, int, "Red channel (0-255) of cast shadow."),
        "colG": ("ShadowGreen", True, int, "Green channel (0-255) of cast shadow."),
        "colB": ("ShadowBlue", True, int, "Blue channel (0-255) of cast shadow."),
        "beginDist": (
            "ShadowStartDistance",
            True,
            float,
        ...
        "endDist": ("ShadowEndDistance", True, float, "Maximum distance (m) at which shadows are cast."),
        "calibulateFar": (
        ...
        "fadeBeginDist": ("FadeStartDistance", True, float, "Shadow starts fading at this distance."),
