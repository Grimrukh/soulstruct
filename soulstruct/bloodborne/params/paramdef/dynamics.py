...
class ItemLotReference(DynamicParamField):
    ...
    def __call__(self, data: ITEMLOT_PARAM_ST):
        index_str = "" if self.index == 0 else str(self.index)
        try:
            item_type = getattr(data, f"Item{index_str}Category")
        ...
        if item_type == ITEMLOT_ITEMCATEGORY.Weapon:
            return (
                WeaponParam,
                "Weapon",
                f"Item slot {index_str} (Weapon).",
            )
        elif item_type == ITEMLOT_ITEMCATEGORY.Armor:
            return (
                ArmorParam,
                "Armor",
                f"Item slot {index_str} (Armor).",
            )
            ...
            return (
                AccessoryParam,
                "Ring",
                f"Item slot {index_str} (Ring).",
            )
        else:
            return (
                int,
                "Unknown",
                f"Item slot {index_str} (unknown item type {item_type}).",
            )
