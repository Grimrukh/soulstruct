...
def INSIGHT(op_node, comparison_value, condition: int, negate: bool = False):
    comparison_type = NEG_COMPARISON_NODES[op_node] if negate else COMPARISON_NODES[op_node]
    return compile_instruction("IfPlayerInsightAmountComparison", condition, comparison_value, comparison_type)
...
def HasItem(item: ItemTyping, condition, item_type=None, negate=False):
    if item_type is None:
        try:
            ...
            raise ValueError(
                "Can only use auto-detecting HasItem() on declared item types (Weapon, Armor, Rune, Good)."
            )
...
@negate_only
def HasRune(ring: AccessoryParam, condition, negate=False):
    return compile_instruction("IfPlayerItemStateExcludingStorage", condition, ring, not negate, ItemType.Rune)
...
def ItemDropped(item: ItemTyping, condition, item_type=None):
    if item_type is None:
        try:
            ...
            raise ValueError("Can only use ItemDropped() on declared item types (Weapon, Armor, Rune, Good).")
...
def OwnsItem(item: ItemTyping, condition, item_type=None, negate=False):
    if item_type is None:
        try:
            ...
            raise ValueError("Can only use OwnsItem() on declared item types (Weapon, Armor, Rune, Good).")
...
@negate_only
def OwnsRune(ring: AccessoryParam, condition, negate=False):
    return compile_instruction("IfPlayerItemStateIncludingStorage", condition, ring, not negate, ItemType.Rune)
