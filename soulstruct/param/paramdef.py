    ...
    def __init__(self, paramdef_bnd_source=None):
        ...
        if paramdef_bnd_source.lower() in {'ptd', 'dsr'}:
            ...
            paramdef_bnd_source = PACKAGE_PATH(
                'param/resources/paramdef.paramdefbnd' + ('.dcx' if paramdef_bnd_source.lower() == 'dsr' else ''))
            if not os.path.isfile(paramdef_bnd_source):
                raise FileNotFoundError("Could not find bundled ParamDef files in 'soulstruct/param/resources'.")
