...
PYTHON_27 = 'G:\\Code\\Anaconda3\\envs\\py27\\python.exe'  # Your Python 2 executable
REBUILDER = '"G:\\Code\\DarkSoulsMods\\Daughters of Ash\\Events\\Scripts27\\command_line.py"'  # HPR's script
...
# 8
def end_if_inside_area(area_id, block_id):
    # TODO: This is a guess.
    return terminate_if_area_state(0, 1, area_id, block_id)


def end_if_outside_area(area_id, block_id):
    # TODO: This is a guess.
    return terminate_if_area_state(0, 0, area_id, block_id)


def restart_if_inside_area(area_id, block_id):
    # TODO: This is a guess.
    return terminate_if_area_state(1, 1, area_id, block_id)


def restart_if_outside_area(area_id, block_id):
    # TODO: This is a guess.
...
def if_event_value_greater_than(output_condition, event_flag_id, number_bits, comparison_value):
    return if_event_value_comparison(output_condition, event_flag_id, number_bits, 4, comparison_value)
...
def if_player_owns_good(output_condition, good):
    if_player_owns_item(output_condition, ItemType.good, good)


def if_player_owns_ring(output_condition, ring):
    if_player_owns_item(output_condition, ItemType.ring, ring)


def if_player_owns_weapon(output_condition, weapon):
    if_player_owns_item(output_condition, ItemType.weapon, weapon)


def if_player_owns_armor(output_condition, armor):
    if_player_owns_item(output_condition, ItemType.armor, armor)


def if_player_does_not_own_good(output_condition, good):
    if_player_does_not_own_item(output_condition, ItemType.good, good)


def if_player_does_not_own_ring(output_condition, ring):
    if_player_does_not_own_item(output_condition, ItemType.ring, ring)


def if_player_does_not_own_weapon(output_condition, weapon):
    if_player_does_not_own_item(output_condition, ItemType.weapon, weapon)


def if_player_does_not_own_armor(output_condition, armor):
    if_player_does_not_own_item(output_condition, ItemType.armor, armor)


def if_player_owns_item(output_condition, item_type, item_id):
    # Includes Bottomless Box.
    if_player_does_or_does_not_own_item(output_condition, item_type, item_id, 1)


def if_player_does_not_own_item(output_condition, item_type, item_id):
    # Includes Bottomless Box.
    if_player_does_or_does_not_own_item(output_condition, item_type, item_id, 0)
